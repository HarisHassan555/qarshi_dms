...
export class FormBuilderComponent implements OnInit {
  ...
  initializeForm() {
    this.formBuilderForm = this.fb.group({
      ...
      selectedUsers: [[]], // Multi-select users
  ...
  editForm(form: CustomForm) {
    ...
    this.formBuilderForm.patchValue({
      ...
      selectedUsers: form.txtUserIds ? form.txtUserIds.split(',').map(id => Number(id)) : []
    });
  ...
  getUserCount(form: CustomForm): number {
    ...
    return form.txtUserIds.split(',').filter(id => id.trim().length > 0).length;
  }
