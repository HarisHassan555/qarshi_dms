...
export class OrderDetailComponent {
   /* constructor() {}
    ...
    userInfo :{} = {}
    txtRoleName :string |undefined;
    constructor(private saleOrderService: SaleOrderService, private route: ActivatedRoute,private http: HttpClient,private location: Location) {}

    ngOnInit() {

        this.route.params.subscribe(params => {
            this.serDealId = params['id']; // Assuming the route param is defined as 'id'
            this.getSaleOrderDetails(this.serDealId);
        });

        /*this.route.queryParams.subscribe(queryParams => {
            this.optionalParam = queryParams['optionalParam'];
        });*/
       /* this.route.queryParams.subscribe(params => {
            this.optionalParam = params['optionalParam'];
            console.log('Optional Param:', this.optionalParam);
        });

        this.route.paramMap.subscribe(params => {
            this.serDealId = params.get('serDealId') || '';
            this.getSaleOrderDetails(this.serDealId);
            this.getSaleOrderAudit(this.serDealId);
        });*/
       ...
       //  this.userInfo = user;
        // @ts-ignore
        this.txtRoleName = user.txtrole;
       /* if("Vendor" === userInfo.txtRoleName){
            hideField = false
        }*/
            ...
            if (data) {
                ...
                data.forEach((item: any) => {
                    ...
                    const processedItem = {
                        ...
                        numQuantity: (item.numAmount && (item.numAmount !== null) && (item.numAmount !== undefined) && item.numAmount !== '')
                            ? (item.numQuantity ?? 0)
                            : 0, // If numAmount is invalid, set numQuantity to 0
                        numItemPrice: item.numItemPrice ?? 0,
                        sapLineItem:item.sapLineItem ?? 'N/A',
                        numTotalPrice: (item.numAmount && (item.numQuantity ?? 0) && (item.numItemPrice ?? 0))
                            ? (item.numQuantity * item.numItemPrice) : '' // 0.251 * 200000
                    };
    ...
    getSaleOrderAudit(id: string) {
        this.saleOrderService.getSaleOrderDetailAudit(id).subscribe((data: any) => {
            console.log('Raw Data:', data);
            if (data) {
                ...
                data.forEach((item: any) => {
                    // @ts-ignore
                    if("VENDOR" ===  this.txtRoleName) {
                        if (item.blIsVendor == true) {
                ...
                console.log('Audit Items:', this.auditItems);
            } else {
                console.error('No data received');
            }
        });
        ...
        if (files) {
            for (let i = 0; i < files.length; i++) {
                ...
                if (file.size <= 35 * 1024 * 1024) {
                    ...
                    alert(`${file.name} exceeds the 35MB size limit and was not uploaded.`);
                }
            }
        }
    }
