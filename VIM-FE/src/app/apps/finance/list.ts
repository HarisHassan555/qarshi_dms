    ...
    getSaleOrder() {
        ...
        this.saleOrderService
            ...
            .subscribe((data: any) => {
                if (data) {
                    // @ts-ignore
                    this.items = data.filter(item => item.numLevel === 4 && item.txtLevel === 'FOURTH'  && (item.txtStatus4 === 'Pending' || item.txtStatus4 === null ||  item.txtStatus4 === 'Hold'));
                    ...
                    // @ts-ignore
                    this.isFinanceHead = user.cfgTblRole?.txtRoleName === 'FINANCE_HEAD';
        ...
        if (this.status == 'Hold' || this.status == 'cancel' || this.status == 'Unhold' || this.status == 'send') {
            ...
            if (this.selectedItem) {
                const payload = {
                    ...
                    txtFinanceRemarks: this.remarks,
                    level: 4,
            ...
            this.saleOrderService.searchSaleInvoice(this.dteFrom, this.dteTo).subscribe((data: any) => {
                if (data) {
                    ...
                    if (this.status === 'Unhold') {
                        ...
                        // @ts-ignore
                        this.slsTblSaleOrder.numLevel = 3;
                        // @ts-ignore
                        this.slsTblSaleOrder.txtLevel = 'THIRD';
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus4 = 'Send Back';

                        // @ts-ignore
                        this.slsTblSaleOrder.txtoriginalStatus = 'Send Back';

                        // @ts-ignore
                        this.slsTblSaleOrder.dteApproveddate3 = null;
                        ...
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus4 = this.status;
    ...
    loadPermissionRoles(roleId: number | undefined, userId: number): void {
        ...
        this.menuService.getAllSubMenuRoles(roleId, userId).subscribe({
            next: (data) => {
                ...
                const filterCriteria = 'Finance Approval';
