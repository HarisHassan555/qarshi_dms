...
export class ProcurementListComponent {
    ...
    prepareStatusChange(item: any, status: string) {
        ...
        this.status = status;
        if (status === 'cancel' || this.status == 'Hold') {
            this.isApproveClicked = false;
            this.modalCancel.open();
        }else if(status === 'approve'){
            this.isApproveClicked = true;
            this.isApproveClicked = this.isInvalidProcurementNoField(this.selectedItem)
            this.modal.open();
        }else {
            this.isApproveClicked = false;
            this.modal.open();
            ...
            .subscribe((data: any) => {
                if (data) {
                    ...
                    this.items = data.filter(item =>
                        item.numLevel === 2 &&
                        item.txtLevel === 'SECOND' &&
                        (item.txtSoapReturnType !== 'E' && item.txtSoapReturnType !== '') &&
                        (item.txtStatus2 === 'Pending' || item.txtStatus2 === 'Hold')
                    );
                    ...
                    // @ts-ignore
                    this.isProcurementHead = user.cfgTblRole?.txtRoleName === 'PROCUREMENT_HEAD';
    ...
    submitStatusChange() {
        ...
        if ((!this.remarks || this.remarks.trim() === '') &&
            (!this.txtSapInvoiceNo || this.txtSapInvoiceNo.trim() === '' || this.txtSapInvoiceNo.length !== 10)) {

            // Check for 'cancel' or 'Hold' status and open appropriate modal
            ...
            this.notificationService.showMessage("Please provide valid remarks and Please provide Procurement no and ensure the Procurement No. is exactly 10 characters long.", 'danger');
            return;
        }else if (this.txtSapInvoiceNo.length !== 10) {
            if (this.status === 'send' || this.status === 'cancel' || this.status == 'Hold' ||  this.status == 'Unhold'){
                ...
                if (this.status === 'approve') {
                    ...
                    if (this.isInvalidProcurementNoField(this.selectedItem)) {
                        this.notificationService.showMessage("Procurement No. must be exactly 10 characters long.", 'danger');
                        return;
                    }
        ...
        /*if (!this.remarks || this.remarks.trim() === '' && (!this.txtSapInvoiceNo || this.txtSapInvoiceNo.trim() === '')) {
            if(this.status == 'cancel' || this.status == 'Hold'){
                this.modalCancel.open();
            }else{
                this.modal.open();
        ...
        if(this.status == 'approve') {
            ...
            }else if (this.isInvalidProcurementNo()) {
                this.notificationService.showMessage("Please provide Procurement No.", 'danger');
                return;
            }
        }
        ...
        if (this.status == 'Hold' || this.status == 'cancel' || this.status == 'Unhold' || this.status == 'send') {
            ...
            if (this.selectedItem) {
                const payload = {
                    ...
                    txtSapInvoiceNo:this.txtSapInvoiceNo,
                    txtProcurementRemarks: this.remarks,
                    level: 2,
            ...
            this.saleOrderService.searchSaleInvoice(this.dteFrom, this.dteTo).subscribe((data: any) => {
                if (data) {
                    ...
                    if (this.status === 'Unhold') {
                        ...
                        // @ts-ignore
                        this.slsTblSaleOrder.numLevel = 1;
                        // @ts-ignore
                        this.slsTblSaleOrder.txtLevel = 'First';
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus2 = 'Send Back';
                        // @ts-ignore
                        this.slsTblSaleOrder.dteApproveddate1 = null;
                        ...
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus2 = this.status;
    ...
    isInvalidProcurementNo() {
        if(this.isApproveClicked){
            return !this.txtSapInvoiceNo;
        }else{
            return;
        }
    ...
    isInvalidProcurementNoField(item:any): boolean {
        return this.isApproveClicked && (!item.txtSapInvoiceNo || item.txtSapInvoiceNo.length !== 10);
    }
    ...
    /*downloadDocument(documentId: string) {
        this.documentService.downloadDocument(documentId).subscribe(
            (data) => {
                ...
                const pdf = new jsPDF();
                debugger;
                pdf.text('Your PDF content here', 10, 10);
                const footerImage = new Image();
                footerImage.src = 'https://placebear.com/g/200/200';
                debugger;
                footerImage.onload = () => {
                    const imgWidth = 40;
                    const imgHeight = 20;
                    const xPosition = (pdf.internal.pageSize.width - imgWidth) / 2;
                    const yPosition = pdf.internal.pageSize.height - imgHeight - 10;
