...
export class AuditListComponent {
    ...
    getSaleOrder() {
        ...
        this.saleOrderService
            ...
            .subscribe((data: any) => {
                if (data) {
                    // @ts-ignore
                    this.items = data.filter(item => item.numLevel === 5 && item.txtLevel === 'FIFTH'  && (item.txtStatus5 === 'Pending' || item.txtStatus5 === null ||  item.txtStatus5 === 'Hold'));
                    ...
                    // @ts-ignore
                    this.isTaxHead = user.cfgTblRole?.txtRoleName === 'AUDIT_HEAD';
    ...
    submitHoldCancelChange() {
        ...
        if (this.selectedItem && this.remarks) {
            // Fetch the sale order from the server
            this.saleOrderService.searchSaleInvoice(this.dteFrom, this.dteTo).subscribe((data: any) => {
                if (data) {
                    // Find the selected sale order by serSaleOrderId
                    // @ts-ignore
                    ...
                    if (this.status === 'Unhold') {
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus5 = 'Pending';
                        ...
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus4 = 'cancel';
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus5 = 'cancel';
                       ...
                       // this.slsTblSaleOrder.txtStatus3 = 'Pending';
                        // @ts-ignore
                        this.slsTblSaleOrder.numLevel = 4;
                        // @ts-ignore
                        this.slsTblSaleOrder.txtLevel = 'FOURTH';
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus4 = 'Pending';

                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus5 = 'Send Back';
                        ...
                        // @ts-ignore
                        this.slsTblSaleOrder.txtStatus5 = this.status;
                        // @ts-ignore
                        this.slsTblSaleOrder.blIsVendor = this.statusChecked;
                    ...
                    this.saleOrderService.updateSaleOrderFromSap(this.slsTblSaleOrder).subscribe(
                        (response: any) => {
                            ...
                            if (response === 'Success') {
                                ...
                                this.notificationService.showMessage("Sale order updated successfully.", 'success');
                                // Refresh the sale order list
                                // Reset selected item and remarks
                                this.selectedItem = null;
                                this.remarks = '';
                                this.searchSaleInvoice(this.dateFrom, this.dateTo);
