...
export class MarketingListComponent {
   ...
   formatDateLocale(date: Date | null): string | null {
    ...
    loadPermissionRoles(roleId: number | undefined, userId: number): void {
        ...
        this.menuService.getAllSubMenuRoles(roleId, userId).subscribe({
            next: (data) => {
                ...
                const filterCriteria = 'Marketing Approval';
    ...
    /* async modifyPdf() {
         ...
         const pdfDoc = await PDFDocument.load(await existingPdfBytes);
    ...
    download(pdfBytes: Uint8Array, filename: string, mimeType: string) {
