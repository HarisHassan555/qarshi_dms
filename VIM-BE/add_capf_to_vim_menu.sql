-- =====================================================
-- SQL Script: Add CAPF Submenu to VIM Menu
-- Database: vim_3
-- Description: This script ensures CAPF submenu is properly
...
-- =====================================================
-- Step 2: Check if CAPF submenu already exists
-- =====================================================
SET @capf_submenu_id = (
...
SELECT 
    CASE 
        WHEN @capf_submenu_id IS NOT NULL THEN CONCAT('CAPF submenu EXISTS with ID: ', @capf_submenu_id)
        ELSE 'CAPF submenu does NOT exist - will be created'
    END AS 'CAPF Submenu Status';

-- =====================================================
-- Step 3: Get next order number for CAPF
-- =====================================================
...
-- =====================================================
-- Step 4: Insert or Update CAPF Submenu
-- =====================================================
...
-- Insert CAPF submenu if it doesn't exist
...
-- Update CAPF submenu if it exists
UPDATE cfg_tbl_sub_menu 
SET 
    ser_menu_id = @vim_menu_id,
    txt_sub_menu_url = 'CAPF',
    bl_is_active = 1,
    bln_status = 1,
    bl_is_deleted = 0,
    bl_is_view = 1,
...
WHERE ser_sub_menu_id = @capf_submenu_id
  AND @vim_menu_id IS NOT NULL
  AND @capf_submenu_id IS NOT NULL;

-- =====================================================
-- Step 5: Get CAPF Submenu ID (after insert/update)
-- =====================================================
SET @capf_submenu_id = (
    SELECT ser_sub_menu_id 
    FROM cfg_tbl_sub_menu 
...
SELECT 
    ...
    CASE 
        WHEN @capf_submenu_id IS NOT NULL THEN 'SUCCESS - CAPF submenu ID found'
        ELSE 'ERROR - CAPF submenu ID not found'
    END AS 'Status';

-- =====================================================
-- Step 6: Create Role Permissions for CAPF
-- =====================================================
...
-- =====================================================
-- Step 7: Create User-Specific Permissions for CAPF
-- =====================================================
...
-- =====================================================
-- Verify CAPF submenu was created/updated
SELECT 
...
SELECT 
    CASE 
        ...
        ) THEN '✅ CAPF is correctly configured - RESTART BACKEND to see changes'
        ELSE '❌ CAPF needs to be fixed'
    END AS 'FINAL STATUS';
