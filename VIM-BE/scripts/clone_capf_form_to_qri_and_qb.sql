-- =============================================================================
-- Clone main CAPF form -> CAPF QRI and CAPF QB (staging / production)
-- =============================================================================
...
-- Logo/title: handled in app code by target form name (CAPF QRI / CAPF QB).
-- =============================================================================
...
-- Clone CAPF QRI
-- ---------------------------------------------------------------------------
SET @target_form_name = 'CAPF QRI';
SET @target_form_code = 'CAPF-QRI';
...
SET @new_form_id_qri = LAST_INSERT_ID();
...
WHERE f.ser_form_id = @source_form_id
  ...
  AND @new_form_id_qri IS NOT NULL
  AND @new_form_id_qri > 0;
...
SELECT
    CASE
        ...
        WHEN @new_form_id_qri > 0 THEN CONCAT('CAPF QRI created, ser_form_id=', @new_form_id_qri)
        ELSE 'CAPF QRI already exists - no new row inserted'
    END AS capf_qri_result;

-- ---------------------------------------------------------------------------
-- Clone CAPF QB
-- ---------------------------------------------------------------------------
SET @target_form_name = 'CAPF QB';
SET @target_form_code = 'CAPF-QB';
...
SET @new_form_id_qb = LAST_INSERT_ID();
...
WHERE f.ser_form_id = @source_form_id
  AND COALESCE(f.bl_is_deleted, 0) = 0
  AND @new_form_id_qb IS NOT NULL
  AND @new_form_id_qb > 0;
...
SELECT
    CASE
        WHEN @source_form_id IS NULL THEN 'SKIPPED - source CAPF form not found'
        WHEN @new_form_id_qb > 0 THEN CONCAT('CAPF QB created, ser_form_id=', @new_form_id_qb)
        ELSE 'CAPF QB already exists - no new row inserted'
    END AS capf_qb_result;
...
FROM cfg_tbl_custom_form
WHERE (
        txt_form_name IN ('CAPF Form Hattar', 'CAPF Form', 'CAPF QRI', 'CAPF QB')
     OR txt_form_code IN ('CAPF-0001', 'CAPF-QRI', 'CAPF-QB')
      )
...
WHERE cf.txt_form_name IN ('CAPF Form Hattar', 'CAPF Form', 'CAPF QRI', 'CAPF QB')
  AND COALESCE(f.bl_is_deleted, 0) = 0
