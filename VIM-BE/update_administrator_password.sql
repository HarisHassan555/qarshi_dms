...
-- =====================================================
-- Script to update administrator user password to match taimoor's password
-- =====================================================
...
-- Step 2: Find administrator user (try multiple variations)
SET @admin_user_id = (
    SELECT ser_user_id
    FROM cfg_tbl_user
    WHERE (
        txt_user_name LIKE '%administrator%'
        OR txt_user_name LIKE '%admin%'
        OR txt_user_name = 'Administrator'
        OR txt_user_name = 'administrator'
        OR txt_user_name = 'ADMINISTRATOR'
        OR txt_user_name = 'Admin'
        OR txt_user_name = 'ADMIN'
    )
      AND (bl_is_deleted = false OR bl_is_deleted IS NULL)
    ORDER BY 
        CASE 
            WHEN txt_user_name = 'administrator' THEN 1
            WHEN txt_user_name = 'Administrator' THEN 2
            WHEN txt_user_name = 'ADMINISTRATOR' THEN 3
            WHEN txt_user_name LIKE '%administrator%' THEN 4
            WHEN txt_user_name = 'Admin' THEN 5
            WHEN txt_user_name = 'ADMIN' THEN 6
            ELSE 7
        END
    LIMIT 1
);
...
SELECT 
    @admin_user_id AS 'Administrator User ID',
    @admin_username AS 'Administrator Username',
    CASE
        WHEN @admin_user_id IS NULL THEN 'ERROR - Administrator user not found'
        WHEN @taimoor_password IS NULL THEN 'ERROR - taimoor password not found'
        ELSE 'SUCCESS - Ready to update password'
    END AS 'Status';

-- Step 3: Update administrator password to match taimoor's
UPDATE cfg_tbl_user
SET
...
SELECT 
    CASE
        WHEN @admin_user_id IS NOT NULL AND @taimoor_password IS NOT NULL AND ROW_COUNT() > 0 THEN 
            CONCAT('SUCCESS - Administrator password updated! User: ', @admin_username)
        WHEN @admin_user_id IS NULL THEN 'ERROR - Administrator user not found'
        WHEN @taimoor_password IS NULL THEN 'ERROR - taimoor password not found'
...
-- Step 4: Verification
-- =====================================================
SELECT 'Verifying administrator user:' AS 'Verification';
SELECT 
...
-- =====================================================
-- Step 5: List all potential administrator users (for reference)
-- =====================================================
SELECT 'All potential administrator users:' AS 'Reference';
SELECT 
    ser_user_id AS 'User ID',
    txt_user_name AS 'Username',
    txt_role AS 'Role',
...
SELECT 
    ...
    CASE 
        ...
        THEN CONCAT('SUCCESS - Administrator (', IFNULL(@admin_username, 'N/A'), ') password updated to match taimoor')
        WHEN @admin_user_id IS NULL
        THEN 'ERROR - Administrator user not found. Check the "All potential administrator users" query above.'
