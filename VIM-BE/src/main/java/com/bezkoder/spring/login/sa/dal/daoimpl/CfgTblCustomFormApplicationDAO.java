...
public class CfgTblCustomFormApplicationDAO implements ICfgTblCustomFormApplicationDAO {
    ...
    private static final float EMAIL_DYNAMIC_FOOTER_SIGNATURE_LIFT = 0f;
    /** Landscape A4 is shorter; email overlay needs a higher footer anchor. */
    private static final float EMAIL_DYNAMIC_FOOTER_TABLE_BOTTOM_Y = 20f;
    private static final float EMAIL_DYNAMIC_FOOTER_LANDSCAPE_TABLE_BOTTOM_Y = 34f;
    private static final float EMAIL_DYNAMIC_FOOTER_LANDSCAPE_EXTRA_SIGNATURE_LIFT = 0f;
    /** Matches {@code .xyz-paper} padding in ApplicationPdfService (top right bottom left). */
    private static final float FE_JS_PDF_MARGIN_LEFT_MM = 12f;
    private static final float FE_JS_PDF_MARGIN_RIGHT_MM = 10f;
    private static final float DYNAMIC_FOOTER_CELL_INSET = 3f;
    private static final float FE_JS_PDF_CONTENT_WIDTH_MM = 200f;
    ...
    private static float tableWidthForPage(float pageWidth) {
        float sideMargin = mmToPdfPoints(FE_JS_PDF_MARGIN_MM);
        return Math.max(0f, pageWidth - sideMargin * 2);
    }

    /** Footer table X/width for email overlay — matches xyz-paper asymmetric padding (12 mm left, 10 mm right). */
    private static float[] emailFooterTableLayout(float pageWidth) {
        float tableX = mmToPdfPoints(FE_JS_PDF_MARGIN_LEFT_MM);
        float tableWidth = Math.max(0f, pageWidth - tableX - mmToPdfPoints(FE_JS_PDF_MARGIN_RIGHT_MM));
        return new float[] { tableX, tableWidth };
    }

    private static float[] dynamicFooterCellBounds(float tableX, float colWidth, int slotIndex) {
        float cellX = tableX + colWidth * slotIndex + DYNAMIC_FOOTER_CELL_INSET;
        float cellW = Math.max(0f, colWidth - 2f * DYNAMIC_FOOTER_CELL_INSET);
        return new float[] { cellX, cellW };
    }
    ...
    @Value("${app.approval-reminder.threshold-hours:24}")
    private long approvalReminderThresholdHours;

    @Value("${app.approval-reminder.batch-size:200}")
    private int approvalReminderBatchSize;
    ...
    public String updateApplication(CfgTblCustomFormApplication application) {
        ...
        try {
            ...
            CfgTblUser editorUser = null;
            try {
                editorUserIdForRestart = commonService.getCurrentLoggedInUser();
                if (editorUserIdForRestart != null && editorUserIdForRestart > 0) {
                    editorUser = commonService.getCurrentUser(editorUserIdForRestart);
                }
            } catch (Exception ignored) {
            }
            ...
            boolean capfVendorDetailsChanged = isCapfForm(formForPoEdit)
                    && capfVendorDataChanged(vendorDataBeforeMerge, existingData);
            if (capfVendorDetailsChanged) {
                boolean isCurrentCapfApprover = editorUserIdForRestart != null
                        && editorUserIdForRestart > 0
                        && editorUser != null
                        && isPendingForUserAtCurrentStage(entityManager, existingApplication, editorUserIdForRestart,
                                editorUser);
                if (!isCurrentCapfApprover) {
                    entityManager.getTransaction().rollback();
                    return "Failure: Only the current approver can edit CAPF procurement details";
                }
            }
            if (isCapfForm(formForPoEdit) && editorUserIdForRestart != null && editorUserIdForRestart > 0) {
                if (userHasRole(editorUser, "PO_Approver") && capfVendorDetailsChanged) {
    ...
    private void notifyCurrentLevelApproversOnInitiatorEdit(CfgTblCustomFormApplication application) {
        ...
        try {
            ...
            if (isCapf && currentLevel == 0) {
                ...
                // Individual-user stage: notify specific approver directly.
                if (isIndividualPipelineStage(currentPipeline)) {
                    sendPipelineIndividualApproverEmail(emailEntityManager, dbApp, form, currentPipeline,
                            displayLevel != null ? displayLevel : currentLevel,
                            formName + " Updated - Pending Approval",
                            buildInitiatorEditNotificationRemarks(dbApp), isCapf);
                    emailEntityManager.getTransaction().commit();
                    return;
                }
    ...
    public String updateApplicationPdf(Integer applicationId, byte[] pdfData, String pdfName, String pdfMime,
        ...
        try {
            ...
            String latestTemplateAction = templateBuilderApplication
                    ? resolveLatestTemplateNotificationAction(application)
                    : null;
            ...
            boolean templateStageZeroReplace = templateBuilderApplication && shouldReplaceTemplateStageZero(application,
                    latestTemplateAction);
    ...
    private boolean shouldReplaceTemplateStageZero(CfgTblCustomFormApplication application, String latestTemplateAction) {
        if (application == null) {
            return false;
        }
        byte[] stageZeroPdf = application.getBlbPdfForStage(0);
        if (stageZeroPdf == null || stageZeroPdf.length == 0) {
            return true;
        }
        String normalizedAction = latestTemplateAction != null
                ? latestTemplateAction.trim().toUpperCase(Locale.ROOT)
                : "";
        if ("SUBMITTED".equals(normalizedAction) || "RESUBMITTED_BY_INITIATOR".equals(normalizedAction)) {
            return true;
        }
        String status = application.getTxtStatus() != null ? application.getTxtStatus().trim().toUpperCase(Locale.ROOT) : "";
        return "PENDING".equals(status) || "REJECTED".equals(status) || "NEW".equals(status);
    }
    ...
    public byte[] resolveDownloadablePdf(Integer applicationId) {
        ...
        try {
            ...
            entityManager.getTransaction().begin();
            CfgTblCustomFormApplication managed = entityManager.find(CfgTblCustomFormApplication.class, applicationId);
            if (managed != null && (managed.getBlbPdfData() == null || managed.getBlbPdfData().length == 0)) {
                managed.setBlbPdfData(pdfBytes);
                if (managed.getBlbPdfForStage(0) == null || managed.getBlbPdfForStage(0).length == 0) {
                    managed.setBlbPdfForStage(0, pdfBytes);
                }
                String code = managed.getTxtFormCode();
                managed.setTxtPdfName(buildPdfFileName(form, code));
                managed.setTxtPdfMime("application/pdf");
            ...
            log.error("Error resolving downloadable PDF for applicationId=" + applicationId + ": " + e.getMessage(), e);
            return null;
        } finally {
            if (entityManager.isOpen()) {
                entityManager.close();
    ...
    private boolean isPendingForUserAtCurrentStage(EntityManager entityManager,
        ...
        try {
            ...
            if (currentPipeline != null) {
                pipelineOrder = safeInt(currentPipeline.get("intApprovalOrder"), null);
            ...
            if (requiredUserId != null)
                return requiredUserId.equals(userId)
                        && !capfUserAlreadyApprovedOpenStage(application, isCapf, currentLevel, pipelineOrder,
            ...
            if (isCapf && currentLevel == 0) {
                ...
                if (submitterDeptId != null) {
                    ...
                    departmentName = resolveDepartmentName(entityManager, departmentId, null, departmentNameCache);
                }
            ...
            if (capfUserAlreadyApprovedOpenStage(application, isCapf, currentLevel, pipelineOrder,
                    departmentId, departmentName, userId)) {
                return false;
            }
...
if (entityManager == null || application == null || form == null || !isCapfForm(form)) {
    ...
    private boolean sendPipelineIndividualApproverEmail(EntityManager emailEntityManager,
        ...
        Integer targetUserId = extractPipelineUserId(pipeline);
        if (targetUserId == null) {
            log.warn("Individual pipeline stage missing userId for appId={}", application.getSerApplicationId());
            return false;
        }
        ...
        try {
            ...
            if (individualApprover == null || individualApprover.getTxtAddress() == null
                ...
                if (startedHere && emailEntityManager.getTransaction().isActive()) {
                    emailEntityManager.getTransaction().rollback();
                }
                log.warn("Individual approver not found or has no email, userId={}, appId={}",
                        targetUserId, application.getSerApplicationId());
                return false;
            }

            String formName = form != null && form.getTxtFormName() != null ? form.getTxtFormName() : "Application";
            int levelDisplay = displayLevel != null ? displayLevel : safeInt(pipeline.get("intApprovalOrder"), 1);
            String baseUrl = getBaseUrl();
            String approveUrl = baseUrl + "/approveApplicationFromEmail?applicationId="
            ...
            String sendBackToInitiatorUrl = baseUrl + "/sendBackToInitiatorFromEmail?applicationId="
                    + application.getSerApplicationId() + "&userId=" + individualApprover.getSerUserId();
            String subject = subjectPrefix + " - Level " + levelDisplay + " - "
                    + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
            String approverName = resolveUserName(emailEntityManager, targetUserId, pipeline);
            String html = generateApprovalEmailHtml(
                    approverName,
                    levelDisplay,
                    application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A",
            ...
            log.info("Pipeline individual approver email sent to {} (userId={}) for appId={}",
                    individualApprover.getTxtAddress(), targetUserId, application.getSerApplicationId());
            return true;
        } catch (Exception e) {
            if (startedHere && emailEntityManager.getTransaction().isActive()) {
                emailEntityManager.getTransaction().rollback();
            }
            log.error("Error sending individual approver email for appId={}: {}",
                    application.getSerApplicationId(), e.getMessage(), e);
            return false;
        }
    }

    private List<BudgetApprover> resolveIndividualFooterApproversForLevel(EntityManager entityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form,
            Map<String, Object> appData,
            List<Map<String, Object>> pipelines,
            Integer targetLevel) {
        if (entityManager == null || application == null || targetLevel == null || targetLevel < 0) {
            return java.util.Collections.emptyList();
        }
        try {
            List<Map<String, Object>> footerFields = extractFooterFields(appData);
            java.util.List<Map<String, Object>> approvalFooterFields = new java.util.ArrayList<>();
            if (footerFields != null) {
                for (Map<String, Object> field : footerFields) {
                    String key = field != null && field.get("key") != null
                            ? String.valueOf(field.get("key")).trim().toLowerCase()
                            : "";
                    if ("prepared_by".equals(key)) {
                        continue;
                    }
                    java.util.List<Object> users = extractFooterUsers(field);
                    if (users == null || users.isEmpty()) {
                        continue;
                    }
                    approvalFooterFields.add(field);
                }
            }
            if (targetLevel >= 0 && targetLevel < approvalFooterFields.size()) {
                Map<String, Object> targetField = approvalFooterFields.get(targetLevel);
                String role = targetField != null && targetField.get("label") != null
                        ? String.valueOf(targetField.get("label"))
                        : "APPROVER";
                java.util.LinkedHashMap<Integer, BudgetApprover> recipients = new java.util.LinkedHashMap<>();
                for (Object userObj : extractFooterUsers(targetField)) {
                    BudgetApprover approver = buildBudgetApprover(userObj, role, entityManager);
                    if (approver != null && approver.userId != null) {
                        recipients.put(approver.userId, approver);
                    }
                }
            ...
            log.warn("Unable to resolve individual footer approvers from footer fields for appId={}, targetLevel={}: {}",
                    application.getSerApplicationId(), targetLevel, e.getMessage());
        }

        try {
            List<BudgetApprover> sequence = getBudgetApprovalSequence(appData, entityManager);
            if (sequence != null && targetLevel >= 0 && targetLevel < sequence.size()) {
                BudgetApprover approver = sequence.get(targetLevel);
                if (approver != null && approver.userId != null) {
                    return java.util.Collections.singletonList(approver);
                }
            }
        } catch (Exception e) {
            log.warn("Unable to resolve individual footer approver from sequence for appId={}, targetLevel={}: {}",
                    application.getSerApplicationId(), targetLevel, e.getMessage());
        }
        return java.util.Collections.emptyList();
    }

    private void refreshIndividualFooterPdfForHistory(CfgTblCustomFormApplication application,
            CfgTblCustomForm form,
            Map<String, Object> appData,
            Integer stageIndex,
            String historyJson) {
        if (application == null || form == null || appData == null) {
            return;
        }
        try {
            byte[] basePdf = null;
            int safeStageIndex = stageIndex != null ? Math.max(0, stageIndex) : 0;
            if (safeStageIndex > 0) {
                basePdf = application.getBlbPdfForStage(safeStageIndex);
            }
            ...
            byte[] refreshedPdf = applyDynamicFooterSignaturesToPdf(basePdf, appData, historyJson);
            if (refreshedPdf != null && refreshedPdf.length > 0) {
                application.setBlbPdfData(refreshedPdf);
                application.setBlbPdfForStage(safeStageIndex, refreshedPdf);
            ...
            log.warn("Unable to refresh individual footer PDF for appId={}, stageIndex={}: {}",
                    application.getSerApplicationId(), stageIndex, e.getMessage(), e);
        }
    }
    ...
    public String approveApplication(Integer applicationId, String remarks, Integer approverUserId, String approvedVia,
        ...
        try {
            ...
            Integer historyLevel = capfHistoryLevelForStage(isCapf, currentLevel, pipelineOrder);
            if (isCapf && userAlreadyApprovedStage(approvalHistory, resolvedApproverId,
            ...
            Integer pipelineLevelIndex = historyLevel;
    ...
    public String rejectApplication(Integer applicationId, String remarks, Integer userId) {
        ...
        try {
            ...
            Map<String, Object> appData = parseApplicationData(application);
            boolean hasDynamicFooterFlow = hasDynamicFooterFlow(application);
            boolean isCapf = form != null && isCapfForm(form);
            ...
            Integer actionDepartmentId = null;
            ...
            if (useIndividualPipelineFlow) {
                ...
                try {
                    ...
                    if (!pipelines.isEmpty() && pipelineIndex >= 0 && pipelineIndex < pipelines.size()) {
                        java.util.Map<String, Object> currentPipeline = pipelines.get(pipelineIndex);
                        if (currentPipeline != null) {
                            actionPipelineOrder = safeInt(currentPipeline.get("intApprovalOrder"), null);
                            if ("individual".equalsIgnoreCase(String.valueOf(currentPipeline.get("type")))) {
                                ...
                                if (deptIdObj != null) {
                                    actionDepartmentId = deptIdObj instanceof Integer ? (Integer) deptIdObj
                                            : Integer.parseInt(deptIdObj.toString());
                    ...
                    } else if (actionDepartmentId != null) {
                        Integer userDepartmentId = loadUserDepartmentId(entityManager, resolvedApproverId);
                        if (userDepartmentId == null || !actionDepartmentId.equals(userDepartmentId)) {
                            entityManager.getTransaction().rollback();
                            return "Failure: You are not authorized to perform this action at this stage";
                        }
                        if (isCapf && currentLevel != null && currentLevel == 0
            ...
            try {
                ...
                if (hasDynamicFooterFlow) {
                    refreshIndividualFooterPdfForHistory(application, form, appData, currentLevel, updatedHistoryJson);
                }
            } catch (Exception e) {
    ...
    public String sendBackApplication(Integer applicationId, String remarks, Integer userId) {
        ...
        try {
            ...
            if (isCapf && userAlreadyApprovedStage(approvalHistory, currentUserId,
        ...
        try {
            ...
            try {
                if (useIndividualPipelineFlow) {
                    ...
                    // the actual first workflow approver, not a flattened footer-sequence fallback.
                    Integer firstApproverUserId = resolveFirstApproverAfterInitiatorResubmit(entityManager, application,
                            form, appData);
                    if (firstApproverUserId != null) {
                        BudgetApprover firstApprover = buildBudgetApprover(
                                java.util.Collections.singletonMap("serUserId", firstApproverUserId),
                                "APPROVER",
                                entityManager);
                        ...
                        log.warn("Skipping send-back-to-initiator email: first workflow approver could not be resolved for appId={}",
                                application.getSerApplicationId());
                    }
                } else {
    ...
    public String sendTemplatePostApprovalEmails(Integer applicationId) {
        ...
        try {
            ...
            String latestTemplateAction = isTemplateBuilderApplication
                    ? resolveLatestTemplateNotificationAction(application)
                    : null;
            String latestTemplateRemarks = isTemplateBuilderApplication
                    ? resolveLatestTemplateNotificationRemarks(application)
                    : null;
            int templateNotificationPipelineIndex = isTemplateBuilderApplication
                    ? resolveTemplateNotificationPipelineIndex(application, currentLevel, latestTemplateAction)
                    : currentLevel;
            List<Map<String, Object>> pipelines = loadEffectiveApprovalPipeline(application, form, entityManager);
            ...
            if (isTemplateBuilderApplication) {
                Map<String, Object> activeOpinionRequest = getActiveTemplateOpinionRequest(appData);
                if (!activeOpinionRequest.isEmpty()
                        && "OPINION_PENDING".equalsIgnoreCase(application.getTxtStatus())) {
                    sendTemplateSubmitterProgressEmail(application, form, currentLevel, null, null, null,
                            application.getTxtStatus(), latestTemplateRemarks);
                    sendTemplateOpinionRequestedEmail(application, form, currentLevel, activeOpinionRequest,
                            null, null, null, latestTemplateRemarks);
                    return "Success";
                }
                if ("OPINION_APPROVED".equalsIgnoreCase(latestTemplateAction)
                        || "OPINION_REJECTED".equalsIgnoreCase(latestTemplateAction)) {
                    sendTemplateSubmitterProgressEmail(application, form, currentLevel, null, null, null,
                            application.getTxtStatus(), latestTemplateRemarks);
                    sendTemplateOpinionReturnEmail(application, form, currentLevel, appData,
                            null, null, null, latestTemplateRemarks);
                    return "Success";
                }
                sendTemplateSubmitterProgressEmail(application, form, currentLevel, null, null, null,
                ...
                sendTemplatePipelineNextApproverEmails(application, form, pipelines, templateNotificationPipelineIndex, null, null, null,
                        "SENT_BACK".equalsIgnoreCase(latestTemplateAction) ? latestTemplateAction : null,
                        latestTemplateRemarks);
                if ("COMPLETED".equalsIgnoreCase(application.getTxtStatus())
                        || "APPROVED".equalsIgnoreCase(application.getTxtStatus())) {
                    sendFinalInitiatorEmail(application);
                }
    ...
    public String sendTemplatePostApprovalEmails(Integer applicationId, byte[] initiatorPdf, byte[] approverPdf,
        ...
        try {
            ...
            if (isTemplateBuilderApplication) {
                ...
                if (!activeOpinionRequest.isEmpty()
                        && "OPINION_PENDING".equalsIgnoreCase(application.getTxtStatus())) {
                    sendTemplateSubmitterProgressEmail(application, form, currentLevel, initiatorBytes, pdfName,
                            pdfMime, application.getTxtStatus(), latestTemplateRemarks);
                    sendTemplateOpinionRequestedEmail(application, form, currentLevel, activeOpinionRequest,
                            approverBytes, pdfName, pdfMime, latestTemplateRemarks);
                    return "Success";
                }
                if ("OPINION_APPROVED".equalsIgnoreCase(latestTemplateAction)
                        || "OPINION_REJECTED".equalsIgnoreCase(latestTemplateAction)) {
            ...
            sendTemplatePipelineNextApproverEmails(application, form, pipelines, templateNotificationPipelineIndex, approverBytes, pdfName,
                    pdfMime, isTemplateBuilderApplication && "SENT_BACK".equalsIgnoreCase(latestTemplateAction)
                            ? latestTemplateAction
                            : null,
                    latestTemplateRemarks);
            if ("COMPLETED".equalsIgnoreCase(application.getTxtStatus())
    ...
    private void sendTemplateSubmitterProgressEmail(CfgTblCustomFormApplication application, CfgTblCustomForm form,
            int currentLevel, byte[] pdfBytes, String pdfName, String pdfMime, String statusOverride,
        ...
        try {
            ...
            String effectiveStatus = statusOverride != null && !statusOverride.trim().isEmpty()
                    ? statusOverride.trim()
                    : application.getTxtStatus();
            int displayLevel = isTemplateBuilderApplication
                    ? resolveTemplateNotificationDisplayLevel(application, currentLevel, effectiveStatus)
                    : currentLevel;
            String subject = formName + " " + resolveTemplateNotificationSubjectSuffix(effectiveStatus) + " - "
                    + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
            String historyJson = isTemplateBuilderApplication
                    ? resolveTemplateNotificationHistoryJson(application)
                    : application.getTxtApprovalHistory();
            String html = generateApprovalEmailHtml(
                    ...
                    formName,
                    effectiveStatus,
                    remarksOverride != null ? remarksOverride : application.getTxtRemarks(),
                    false,
                    null,
                    null,
                    null,
    ...
    private void sendTemplatePipelineNextApproverEmails(CfgTblCustomFormApplication application, CfgTblCustomForm form,
            ...
            String statusOverride, String remarksOverride) {
        ...
        try {
            ...
            java.util.Set<Integer> recipientIds = resolveTemplatePipelineRecipientIds(em, application, pipeline);
            ...
            String effectiveStatus = statusOverride != null && !statusOverride.trim().isEmpty()
                    ? statusOverride.trim()
                    : application.getTxtStatus();
            String subject = formName + " " + resolveTemplateApproverNotificationSubjectSuffix(effectiveStatus, displayLevel) + " - "
                    + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
            String historyJson = isTemplateBuilderApplication
                    ? resolveTemplateNotificationHistoryJson(application)
                    : application.getTxtApprovalHistory();
    ...
    private void sendTemplateOpinionRequestedEmail(CfgTblCustomFormApplication application, CfgTblCustomForm form,
            int currentLevel, Map<String, Object> opinionRequest, byte[] pdfBytes, String pdfName, String pdfMime,
            String remarksOverride) {
        if (application == null || opinionRequest == null || opinionRequest.isEmpty()) {
            return;
        }
        Integer opinionUserId = safeInt(opinionRequest.get("requestedFrom"),
                safeInt(opinionRequest.get("opinionUserId"), null));
        if (opinionUserId == null || opinionUserId <= 0) {
            return;
        }
        sendTemplateTargetedApproverEmail(application, form, opinionUserId, currentLevel,
                "OPINION_PENDING", remarksOverride, pdfBytes, pdfName, pdfMime);
    }

    private void sendTemplateOpinionReturnEmail(CfgTblCustomFormApplication application, CfgTblCustomForm form,
            int currentLevel, Map<String, Object> appData, byte[] pdfBytes, String pdfName, String pdfMime,
            String remarksOverride) {
        if (application == null) {
            return;
        }
        Map<String, Object> opinionRequest = appData != null ? castMap(appData.get("templateOpinionRequest"))
                : java.util.Collections.emptyMap();
        Integer requesterId = safeInt(opinionRequest.get("requestedBy"), application.getSerCurrentApprover());
        if (requesterId == null || requesterId <= 0) {
            requesterId = application.getSerCurrentApprover();
        }
        if (requesterId == null || requesterId <= 0) {
            return;
        }
        sendTemplateTargetedApproverEmail(application, form, requesterId, currentLevel,
                application.getTxtStatus(), remarksOverride, pdfBytes, pdfName, pdfMime);
    }

    private void sendTemplateTargetedApproverEmail(CfgTblCustomFormApplication application, CfgTblCustomForm form,
            Integer recipientUserId, int currentLevel, String statusOverride, String remarksOverride,
            byte[] pdfBytes, String pdfName, String pdfMime) {
        if (application == null || recipientUserId == null || recipientUserId <= 0) {
            return;
        }
        EntityManager em = getEntityManager();
        try {
            em.getTransaction().begin();
            CfgTblUser recipient = em.find(CfgTblUser.class, recipientUserId);
            em.getTransaction().commit();
            if (recipient == null || recipient.getTxtAddress() == null || recipient.getTxtAddress().trim().isEmpty()) {
                return;
            }
            String formName = getResolvedFormName(form);
            ...
            String effectiveStatus = statusOverride != null && !statusOverride.trim().isEmpty()
                    ? statusOverride.trim()
                    : application.getTxtStatus();
            String subject = formName + " "
                    + resolveTemplateApproverNotificationSubjectSuffix(effectiveStatus, displayLevel)
                    + " - " + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
            String baseUrl = getBaseUrl();
            String approveUrl = baseUrl + "/approveApplicationFromEmail?applicationId="
                    + application.getSerApplicationId() + "&userId=" + recipient.getSerUserId();
            ...
            String html = generateApprovalEmailHtml(
                    ...
                    getBaseUrl());
            sendEmailWithSpecificPdf(java.util.Arrays.asList(recipient.getTxtAddress()), subject, html, application,
                    form, false, pdfBytes, pdfName, pdfMime);
        } catch (Exception e) {
            if (em.getTransaction().isActive()) {
                em.getTransaction().rollback();
            }
            log.warn("Failed to send template targeted approver email: {}", e.getMessage());
        } finally {
            if (em.isOpen()) {
                em.close();
            }
    ...
    private String resolveLatestTemplateNotificationAction(CfgTblCustomFormApplication application) {
        if (application == null) {
            return null;
        }
        List<Map<String, Object>> history = filterActiveTemplateHistoryEntries(
        ...
        return application.getTxtStatus();
    }

    private String resolveLatestTemplateNotificationRemarks(CfgTblCustomFormApplication application) {
        if (application == null) {
            return null;
        }
        List<Map<String, Object>> history = filterActiveTemplateHistoryEntries(
        ...
        for (int i = history.size() - 1; i >= 0; i--) {
            ...
            String remarks = entry.get("remarks") != null ? String.valueOf(entry.get("remarks")).trim() : "";
            if (!remarks.isEmpty()) {
                return remarks;
            }
        }
    ...
    private int resolveTemplateNotificationPipelineIndex(CfgTblCustomFormApplication application, int fallbackPipelineIndex,
        ...
        for (int i = history.size() - 1; i >= 0; i--) {
            ...
            return Math.max(0, toLevel - 2);
        }
        return fallback;
    }

    private String resolveTemplateNotificationHistoryJson(CfgTblCustomFormApplication application) {
        if (application == null) {
            return null;
        }
        String priorApprovals = application.getTxtPriorApprovals();
        if (priorApprovals != null && !priorApprovals.trim().isEmpty()) {
            return priorApprovals;
        }
        return application.getTxtApprovalHistory();
    }

    private String resolveTemplateNotificationSubjectSuffix(String status) {
        String normalized = status != null ? status.trim().toUpperCase(Locale.ROOT) : "";
        if ("OPINION_PENDING".equals(normalized) || "OPINION_REQUESTED".equals(normalized)) {
            return "Pending Opinion";
        }
        if ("OPINION_APPROVED".equals(normalized) || "OPINION_REJECTED".equals(normalized)) {
            return "Opinion Submitted";
        }
        if ("SENT_BACK_TO_INITIATOR".equals(normalized)) {
            return "Sent Back To Initiator";
        }
        if ("SENT_BACK".equals(normalized) || "REVISION_REQUIRED".equals(normalized)) {
            return "Sent Back";
        }
        ...
        if ("RESUBMITTED_BY_INITIATOR".equals(normalized)) {
            return "Resubmitted";
        }
        if ("APPROVED".equals(normalized) || "COMPLETED".equals(normalized)) {
            return "Approved";
        }
        return "Updated";
    }

    private String resolveTemplateApproverNotificationSubjectSuffix(String status, int displayLevel) {
        String normalized = status != null ? status.trim().toUpperCase(Locale.ROOT) : "";
        if ("OPINION_PENDING".equals(normalized) || "OPINION_REQUESTED".equals(normalized)) {
            return "Opinion Requested - Requires Your Action - Level " + displayLevel;
        }
        if ("OPINION_APPROVED".equals(normalized) || "OPINION_REJECTED".equals(normalized)) {
            return "Opinion Submitted - Requires Your Action - Level " + displayLevel;
        }
        if ("SENT_BACK".equals(normalized) || "REVISION_REQUIRED".equals(normalized)) {
            return "Sent Back - Requires Your Action - Level " + displayLevel;
        }
        if ("SENT_BACK_TO_INITIATOR".equals(normalized)) {
            return "Returned To Initiator";
        }
        return "Pending Approval - Level " + displayLevel;
    }

    private int resolveTemplateNotificationDisplayLevel(CfgTblCustomFormApplication application,
            int fallbackPipelineIndex, String notificationType) {
        int fallbackLevel = Math.max(1, fallbackPipelineIndex + 1);
        if (application == null) {
            return fallbackLevel;
        }
        String normalizedType = notificationType != null ? notificationType.trim().toUpperCase(Locale.ROOT) : "";
        if ("SENT_BACK_TO_INITIATOR".equals(normalizedType)
                || "SUBMITTED".equals(normalizedType)
                || "RESUBMITTED_BY_INITIATOR".equals(normalizedType)) {
            return 1;
        }
        List<Map<String, Object>> history = filterActiveTemplateHistoryEntries(
                parseApprovalHistory(application.getTxtApprovalHistory()));
    ...
    @SuppressWarnings("unchecked")
    private java.util.Set<Integer> resolveTemplatePipelineRecipientIds(EntityManager em,
            CfgTblCustomFormApplication application,
            Map<String, Object> pipeline) {
        java.util.Set<Integer> recipientIds = new java.util.LinkedHashSet<>();
        if (em == null || application == null || pipeline == null) {
            return recipientIds;
        }

        String stageType = pipeline.get("type") != null ? String.valueOf(pipeline.get("type")).trim() : "";
        String dynamicTarget = pipeline.get("dynamicTarget") != null ? String.valueOf(pipeline.get("dynamicTarget")).trim() : "";

        if (isIndividualPipelineStage(pipeline) || "individual".equalsIgnoreCase(stageType)) {
            ...
            boolean includesInitiator = "initiator".equalsIgnoreCase(dynamicTarget);
        ...
        if ("role".equalsIgnoreCase(stageType)) {
            Integer roleId = safeInt(
                    pipeline.get("serRoleId"),
                    safeInt(castMap(pipeline.get("cfgTblRole")).get("serRoleId"), null));
            if (roleId != null) {
                try {
                    for (CfgTblUser user : em.createQuery(
                            "SELECT u FROM CfgTblUser u WHERE u.cfgTblRole.serRoleId = :roleId "
                                    + "AND (u.blIsDeleted IS NULL OR u.blIsDeleted = false)",
                            CfgTblUser.class)
                            .setParameter("roleId", roleId)
                            .getResultList()) {
                        if (user != null && user.getSerUserId() != null) {
                            recipientIds.add(user.getSerUserId());
                        }
                    }
                } catch (Exception e) {
                    log.warn("Failed to resolve template role-stage recipients for role {}: {}", roleId, e.getMessage());
                }
            }
            return recipientIds;
        }

        Integer departmentId = safeInt(
                pipeline.get("serDepartmentId"),
                safeInt(pipeline.get("departmentId"),
                        safeInt(castMap(pipeline.get("hrTblDepartment")).get("serDepartmentId"), null)));
        String departmentName = resolveDepartmentName(em, departmentId, pipeline);

        if ("initiator_hod".equalsIgnoreCase(dynamicTarget) || isUserDepartmentHodStage(pipeline, departmentName)) {
        ...
        if (department != null && department.getSerDepartmentHeadId() != null) {
            for (String item : department.getSerDepartmentHeadId().split(",")) {
                Integer headId = safeInt(item, null);
        ...
        if (recipientIds.isEmpty()) {
            ...
            if (fallbackHeadId != null && fallbackHeadId > 0) {
                recipientIds.add(fallbackHeadId);
            }
        }
        return recipientIds;
    }
    ...
    private void overlayTemplateApprovalSignaturesOnPdf(PDDocument document,
        ...
        for (Map<String, Object> field : fields) {
            ...
            } else if ("dynamic_approver_name".equals(type)
                    || "dynamic_approval_timestamp".equals(type)
                    || "dynamic_approver_department".equals(type)
                    || "dynamic_approver_designation".equals(type)
                    || "dynamic_user_details".equals(type)) {
    ...
    private boolean templateSignatureEntryMatchesTarget(Map<String, Object> entry,
        ...
        if (targetId != null && !targetId.trim().isEmpty()) {
            return entryStepId != null && !entryStepId.trim().isEmpty()
                    && targetId.equalsIgnoreCase(entryStepId);
        }
    ...
    private void drawTemplateDynamicApprovalTextGroup(PDPageContentStream content,
        ...
        for (String value : values) {
            String[] valueLines = value != null ? value.split("\\r?\\n") : new String[] { "" };
            for (String valueLine : valueLines) {
                String normalizedLine = valueLine != null ? valueLine : "";
                List<String> wrapped = wrapText(normalizedLine, font, fontSize, maxWidth);
    ...
    private String resolveTemplateDynamicApprovalTextValue(String fieldType, Map<String, Object> entry) {
        ...
        if ("dynamic_approver_department".equals(fieldType)) {
            return firstNonBlank(
                    ...
                    extractDepartmentName(entry.get("user")),
                    extractDepartmentName(entry.get("approver")),
                    extractDepartmentName(entry.get("employee")));
        }
        if ("dynamic_approver_designation".equals(fieldType)) {
            return firstNonBlank(
                    entry.get("txtDesignation") != null ? String.valueOf(entry.get("txtDesignation")) : null,
                    entry.get("designation") != null ? String.valueOf(entry.get("designation")) : null,
                    extractDesignation(entry.get("user")),
                    extractDesignation(entry.get("approver")),
                    extractDesignation(entry.get("employee")));
        }
        if ("dynamic_user_details".equals(fieldType)) {
            ...
            if (name != null && !name.trim().isEmpty()) {
                details.add(name.trim());
            }
            if (designation != null && !designation.trim().isEmpty()) {
                details.add(designation.trim());
            }
            if (department != null && !department.trim().isEmpty()) {
                details.add(department.trim());
            }
            if (timestamp != null && !timestamp.trim().isEmpty()) {
                details.add(timestamp.trim());
            }
            return String.join("\n", details);
        }
        return "";
    ...
    private String validateCapfCodeNotUsedElsewhere(EntityManager entityManager, Integer excludeApplicationId,
            String nativeSql, String code, String codeLabel) {
        if (excludeApplicationId == null || code == null || code.trim().isEmpty()) {
            return null;
        }
        try {
            Number count = (Number) entityManager.createNativeQuery(nativeSql)
                    .setParameter("excludeId", excludeApplicationId)
                    .setParameter("code", code.trim())
                    .getSingleResult();
            if (count != null && count.intValue() > 0) {
                return codeLabel + " is already assigned to another application";
            }
        } catch (Exception e) {
            log.warn("Failed to validate {} uniqueness for appId={}: {}", codeLabel, excludeApplicationId,
                    e.getMessage());
            return "Unable to validate " + codeLabel + " uniqueness";
        }
        return null;
    }

    private static final String SQL_ASSET_CODE_IN_USE =
            "SELECT COUNT(*) FROM cfg_tbl_custom_form_application " +
            "WHERE ser_application_id <> :excludeId " +
            "AND txt_asset_code IS NOT NULL AND TRIM(txt_asset_code) <> '' " +
            "AND LOWER(TRIM(txt_asset_code)) = LOWER(TRIM(:code)) " +
            "AND (bl_is_deleted = 0 OR bl_is_deleted IS NULL)";

    private static final String SQL_PR_CODE_IN_USE =
            "SELECT COUNT(*) FROM cfg_tbl_custom_form_application " +
            "WHERE ser_application_id <> :excludeId " +
            "AND txt_pr_code IS NOT NULL AND TRIM(txt_pr_code) <> '' " +
            "AND LOWER(TRIM(txt_pr_code)) = LOWER(TRIM(:code)) " +
            "AND (bl_is_deleted = 0 OR bl_is_deleted IS NULL)";

    private static final String SQL_PO_CODE_IN_USE =
            "SELECT COUNT(*) FROM cfg_tbl_custom_form_application " +
            "WHERE ser_application_id <> :excludeId " +
            "AND txt_po_code IS NOT NULL AND TRIM(txt_po_code) <> '' " +
            "AND LOWER(TRIM(txt_po_code)) = LOWER(TRIM(:code)) " +
            "AND (bl_is_deleted = 0 OR bl_is_deleted IS NULL)";
    ...
    public String assignAssetCode(Integer applicationId, String assetCode, Integer userId, String approvedIp) {
        ...
        try {
            ...
            if (!isFirstAssignment) {
                boolean capfPoReapprovalConfirm = capfPoReapproval && "ASSET_PENDING".equalsIgnoreCase(status)
                        && trimmedCode.equalsIgnoreCase(application.getTxtAssetCode().trim());
                if (!capfPoReapprovalConfirm) {
                    entityManager.getTransaction().rollback();
                    return "Failure: Asset code has already been assigned";
                }
            } else {
                String duplicateMessage = validateCapfCodeNotUsedElsewhere(entityManager,
                        application.getSerApplicationId(), SQL_ASSET_CODE_IN_USE, trimmedCode, "Asset code");
    ...
    public String assignPrCode(Integer applicationId, String prCode, Integer userId, String approvedIp) {
        ...
        try {
            ...
            if (!isFirstAssignment) {
                entityManager.getTransaction().rollback();
                return "Failure: PR code has already been assigned";
            }
            String duplicatePrMessage = validateCapfCodeNotUsedElsewhere(entityManager,
                    application.getSerApplicationId(), SQL_PR_CODE_IN_USE, trimmedCode, "PR code");
            if (duplicatePrMessage != null) {
                entityManager.getTransaction().rollback();
                return "Failure: " + duplicatePrMessage;
            }
    ...
    public String assignPoCode(Integer applicationId, String poCode, Integer userId, String approvedIp) {
        ...
        try {
            ...
            if (!isFirstAssignment) {
                entityManager.getTransaction().rollback();
                return "Failure: PO code has already been assigned";
            }
            String duplicatePoMessage = validateCapfCodeNotUsedElsewhere(entityManager,
                    application.getSerApplicationId(), SQL_PO_CODE_IN_USE, trimmedCode, "PO code");
            if (duplicatePoMessage != null) {
                entityManager.getTransaction().rollback();
                return "Failure: " + duplicatePoMessage;
            }
            ...
            if (pipelines != null && !pipelines.isEmpty() && nextPipelineIndex >= 0 && nextPipelineIndex < pipelines.size()) {
                try {
                    ...
                    if (nextLevelPipeline != null) {
                        if (isIndividualPipelineStage(nextLevelPipeline)) {
                            Integer nextLevelOrder = extractPipelineOrder(nextLevelPipeline, currentLevel + 1);
                            sendPipelineIndividualApproverEmail(emailEntityManager, application, form, nextLevelPipeline,
                                    nextLevelOrder, formName + " Pending Approval", null, isCapf);
                            return;
                        }
    ...
    public void sendPendingApprovalReminderEmails() {
        EntityManager entityManager = getEntityManager();
        try {
            ...
            List<ApprovalReminderCandidate> candidates = entityManager.createQuery(
                    "SELECT a.serApplicationId, a.serFormId, a.txtStatus, a.intCurrentApprovalLevel, "
                            + "a.serSubmittedBy, a.dteCreatedDate, a.dteModifiedDate "
                            + "FROM CfgTblCustomFormApplication a "
                            + "WHERE (a.blIsDeleted = false OR a.blIsDeleted IS NULL) "
                    ...
                    .setMaxResults(Math.max(1, approvalReminderBatchSize))
                    .getResultList()
                    .stream()
    ...
    private ApprovalReminderCandidate mapApprovalReminderCandidate(Object[] row) {
        if (row == null || row.length < 7) {
            return null;
        }
        ApprovalReminderCandidate candidate = new ApprovalReminderCandidate();
        candidate.applicationId = safeInt(row[0], null);
        candidate.formId = safeInt(row[1], null);
        candidate.status = row[2] != null ? String.valueOf(row[2]) : null;
        candidate.currentLevel = safeInt(row[3], null);
        candidate.submittedBy = safeInt(row[4], null);
        candidate.createdDate = row[5] instanceof java.sql.Timestamp ? (java.sql.Timestamp) row[5] : null;
        candidate.modifiedDate = row[6] instanceof java.sql.Timestamp ? (java.sql.Timestamp) row[6] : null;
        return candidate.applicationId != null ? candidate : null;
    }

    private java.sql.Timestamp resolveCurrentStageStartedAt(ApprovalReminderCandidate candidate) {
        if (candidate == null) {
            return null;
        }
        return candidate.modifiedDate != null
                ? candidate.modifiedDate
                : candidate.createdDate;
    }

    private boolean hasApprovalReminderForCurrentStage(EntityManager entityManager,
            Integer applicationId,
            java.sql.Timestamp stageStartedAt) {
        if (entityManager == null || applicationId == null || stageStartedAt == null) {
            return false;
        }
        try {
            Long count = entityManager.createQuery(
                    "SELECT COUNT(l) FROM CfgTblAppActivityLog l "
                            + "WHERE UPPER(COALESCE(l.txtActionType, '')) = :actionType "
                            + "AND UPPER(COALESCE(l.txtEntityType, '')) = :entityType "
                            + "AND l.serEntityId = :entityId "
                            + "AND UPPER(COALESCE(l.txtStatus, '')) = :status "
                            + "AND l.dteCreatedDate >= :stageStartedAt",
                    Long.class)
                    .setParameter("actionType", "APPROVAL_REMINDER")
                    .setParameter("entityType", "APPLICATION")
                    .setParameter("entityId", applicationId)
                    .setParameter("status", "SUCCESS")
                    .setParameter("stageStartedAt", stageStartedAt)
                    .getSingleResult();
            return count != null && count.longValue() > 0L;
        } catch (Exception e) {
            log.warn("Could not determine prior reminder state for appId={}: {}",
                    applicationId, e.getMessage());
            return false;
        }
    }

    private void logApprovalReminder(EntityManager entityManager,
            Integer applicationId,
            CfgTblCustomFormApplication application,
            java.sql.Timestamp stageStartedAt,
            int recipientCount) {
        if (entityManager == null || applicationId == null) {
            return;
        }
        boolean startedHere = false;
        try {
            ...
            CfgTblAppActivityLog activityLog = new CfgTblAppActivityLog();
            activityLog.setTxtActionType("APPROVAL_REMINDER");
            activityLog.setTxtStatus("SUCCESS");
            activityLog.setTxtMessage("Approval reminder email sent");
            activityLog.setTxtEntityType("APPLICATION");
            activityLog.setSerEntityId(applicationId);
            activityLog.setDteCreatedDate(commonService.getCurrentTimeStamp_new());

            Map<String, Object> payload = new java.util.LinkedHashMap<>();
            payload.put("applicationId", applicationId);
            payload.put("status", application != null ? application.getTxtStatus() : null);
            payload.put("currentLevel", application != null ? application.getIntCurrentApprovalLevel() : null);
            payload.put("stageStartedAt", stageStartedAt != null ? stageStartedAt.toString() : null);
            payload.put("recipientCount", recipientCount);
            activityLog.setTxtPayload(new ObjectMapper().writeValueAsString(payload));

            entityManager.persist(activityLog);
            if (startedHere && entityManager.getTransaction().isActive()) {
                entityManager.getTransaction().commit();
            }
        } catch (Exception e) {
            if (startedHere && entityManager.getTransaction().isActive()) {
            ...
            log.warn("Failed to log approval reminder for appId={}: {}",
                    applicationId, e.getMessage());
        }
    }

    private static class ApprovalReminderCandidate {
    ...
    private int sendPendingApprovalReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        if (emailEntityManager == null || application == null) {
            return 0;
        }

        String status = application.getTxtStatus() != null ? application.getTxtStatus().trim().toUpperCase(Locale.ROOT) : "";
        if (status.isEmpty() || "APPROVED".equals(status) || "REJECTED".equals(status) || "COMPLETED".equals(status)) {
            return 0;
        }
        ...
        if ("ASSET_PENDING".equals(status) || "ASSET_CODE_PENDING".equals(status)) {
            return sendFinanceReminderEmail(emailEntityManager, application, form);
        }
        if ("PR_PENDING".equals(status)) {
            return sendPrCodeReminderEmail(emailEntityManager, application, form);
        }
        if ("PO_PENDING".equals(status)) {
            return sendPoCodeReminderEmail(emailEntityManager, application, form);
        }
        if ("PO_VENDOR_TE_PENDING".equals(status)) {
            return sendPoVendorTeReminderEmail(emailEntityManager, application, form);
        }
        return sendWorkflowStageReminderEmail(emailEntityManager, application, form);
    }

    private int sendCeoApprovalReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        int sent = 0;
        boolean startedHere = false;
        try {
            ...
            boolean isCapf = isCapfForm(form);
            String baseUrl = getBaseUrl();
            String subject = "Reminder: CEO Approval Required - "
                    + (application.getTxtFormCode() != null ? application.getTxtFormCode() : formName);
            String remarks = buildApprovalReminderRemarks();
            ...
            for (RoleRecipient recipient : ceoRecipients) {
                ...
                String html = generateApprovalEmailHtml(
                        ...
                        baseUrl);
                sendEmailWithInlineFormPreview(
                        java.util.Collections.singletonList(recipient.email),
                        subject,
                        html,
                        application,
                        form,
            ...
            log.warn("Failed CEO reminder email for appId={}: {}", application.getSerApplicationId(), e.getMessage());
            return 0;
        }
        return sent;
    }

    private int sendFinanceReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        boolean startedHere = false;
        try {
            ...
            boolean isCapf = isCapfForm(form);
            String assignUrl = getBaseUrl() + "/assign-asset-code/" + application.getSerApplicationId();
            String subject = "Reminder: Finance Action Required (Asset Code) - "
                    + (application.getTxtFormCode() != null ? application.getTxtFormCode() : formName);
            StringBuilder html = new StringBuilder();
            html.append("<p>Dear Finance Team,</p>");
            html.append("<p>").append(escapeHtml(buildApprovalReminderRemarks())).append("</p>");
            html.append("<p>The application <strong>")
                    .append(escapeHtml(application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A"))
                    .append("</strong> is awaiting asset code assignment.</p>");
            html.append("<p><a href='").append(assignUrl)
                    .append("' style='padding:10px 16px;background:#2c7be5;color:#fff;text-decoration:none;border-radius:4px;'>Assign Asset Code</a></p>");
            ...
            return financeEmails.size();
        } catch (Exception e) {
            if (startedHere && emailEntityManager.getTransaction().isActive()) {
                emailEntityManager.getTransaction().rollback();
            }
            log.warn("Failed finance reminder email for appId={}: {}", application.getSerApplicationId(), e.getMessage());
            return 0;
        }
    }

    private int sendPrCodeReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        boolean startedHere = false;
        try {
            ...
            String html = generateApprovalEmailHtml(
                    ...
                    application.getTxtApprovalHistory(),
                    getBaseUrl());
            html = appendPrCodeActionButtonForInitiator(html, application);
            sendEmailWithInlineFormPreview(
                    java.util.Collections.singletonList(submitter.getTxtAddress()),
                    subject,
                    html,
                    application,
                    form,
            ...
            log.warn("Failed PR code reminder email for appId={}: {}", application.getSerApplicationId(), e.getMessage());
            return 0;
        }
    }

    private int sendPoCodeReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        boolean startedHere = false;
        try {
            ...
            boolean isCapf = isCapfForm(form);
            String assignUrl = getBaseUrl() + "/po-code/" + application.getSerApplicationId();
            String subject = "Reminder: " + formName + " Awaiting PO Code - "
                    + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
            StringBuilder html = new StringBuilder();
            html.append("<p>Dear PO Approver,</p>");
            html.append("<p>").append(escapeHtml(buildApprovalReminderRemarks())).append("</p>");
            html.append("<p>The application <strong>")
                    ...
                    .append("</strong> is awaiting PO code assignment.</p>");
            html.append("<p><a href='").append(assignUrl)
                    .append("' style='padding:10px 16px;background:#2c7be5;color:#fff;text-decoration:none;border-radius:4px;'>Assign PO Code</a></p>");
            sendEmailWithInlineFormPreview(poEmails, subject, html.toString(), application, form, isCapf,
                    isCapf ? "capf-inline" : "form-inline");
            if (startedHere && emailEntityManager.getTransaction().isActive()) {
                emailEntityManager.getTransaction().commit();
            }
            return poEmails.size();
        } catch (Exception e) {
            if (startedHere && emailEntityManager.getTransaction().isActive()) {
                emailEntityManager.getTransaction().rollback();
            }
            log.warn("Failed PO code reminder email for appId={}: {}", application.getSerApplicationId(), e.getMessage());
            return 0;
        }
    }

    private int sendPoVendorTeReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        int sent = 0;
        boolean startedHere = false;
        try {
            ...
            Integer teDeptId = resolveCapfTechnicalExpertDepartmentId(emailEntityManager, application, form);
            if (teDeptId == null) {
                if (startedHere && emailEntityManager.getTransaction().isActive()) {
                    emailEntityManager.getTransaction().rollback();
                }
            ...
            String formName = getResolvedFormName(form);
            String baseUrl = getBaseUrl();
            String viewApplicationUrl = baseUrl + "/application-details/" + application.getSerApplicationId() + "?from=pending";
            String remarks = buildApprovalReminderRemarks() + " Vendor details were updated after PO assignment. Technical Expert review is required.";
            ...
            for (Integer headId : headIds) {
                ...
                String subject = "Reminder: " + formName + " - PO Vendor Edit Review - "
                        + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
                String html = generatePoVendorTeReviewEmailHtml(
                        ...
                        application.getTxtApprovalHistory(),
                        baseUrl);
                emailService.sendHtmlEmail(java.util.Collections.singletonList(approver.getTxtAddress()), subject, html);
                sent++;
            }
            ...
            log.warn("Failed Technical Expert reminder email for appId={}: {}",
                    application.getSerApplicationId(), e.getMessage());
            return 0;
        }
        return sent;
    ...
    private int sendWorkflowStageReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        if (form == null) {
            return 0;
        ...
        boolean isCapf = isCapfForm(form);
        String formName = getResolvedFormName(form);
        String reminderRemarks = buildApprovalReminderRemarks();

        if (isCapf && application.getIntCurrentApprovalLevel() != null && application.getIntCurrentApprovalLevel() == -1) {
            Integer initialSignerId = extractInitialSignerId(application);
            if (initialSignerId == null) {
                return 0;
            }
            ...
            try {
                ...
                String baseUrl = getBaseUrl();
                String approveUrl = baseUrl + "/approveApplicationFromEmail?applicationId="
                        + application.getSerApplicationId() + "&userId=" + initialSigner.getSerUserId();
                String rejectUrl = baseUrl + "/rejectApplicationFromEmail?applicationId="
                        + application.getSerApplicationId() + "&userId=" + initialSigner.getSerUserId();
                String sendBackUrl = baseUrl + "/sendBackApplicationFromEmail?applicationId="
                        + application.getSerApplicationId() + "&userId=" + initialSigner.getSerUserId();
                String sendBackToInitiatorUrl = baseUrl + "/sendBackToInitiatorFromEmail?applicationId="
                        + application.getSerApplicationId() + "&userId=" + initialSigner.getSerUserId();
                String subject = "Reminder: " + formName + " Initial Signature Required - "
                        + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
                String html = generateApprovalEmailHtml(
                        initialSigner.getTxtUserName() != null ? initialSigner.getTxtUserName() : "User",
                        0,
                        ...
                        baseUrl);
                sendEmailWithInlineFormPreview(
                        java.util.Collections.singletonList(initialSigner.getTxtAddress()),
                        subject,
                        html,
                        application,
                        form,
                ...
                log.warn("Failed initial signer reminder email for appId={}: {}",
                        application.getSerApplicationId(), e.getMessage());
                return 0;
            }
        }
        ...
        if (useIndividualPipelineFlow) {
            ...
            BudgetApprover approver = sequence.get(currentLevel);
            ...
            try {
                ...
                String html = generateApprovalEmailHtml(
                        ...
                        baseUrl);
                sendEmailWithInlineFormPreview(
                        java.util.Collections.singletonList(approver.email),
                        subject,
                        html,
                        application,
                        form,
                ...
                log.warn("Failed individual-stage reminder email for appId={}: {}",
                        application.getSerApplicationId(), e.getMessage());
                return 0;
            }
        }

        List<Map<String, Object>> pipelines = loadEffectiveApprovalPipeline(application, form, emailEntityManager);
        int currentLevel = currentLevelSafe(application);
        int pipelineIndex = isCapf ? currentLevel - 1 : currentLevel;
        ...
        if (isCapf && currentLevel == 0) {
            ...
            return sendDepartmentStageReminderEmail(emailEntityManager, application, form, submitterDeptId, null, 0,
                    formName, reminderRemarks, isCapf);
        }

        if (pipelineIndex < 0 || pipelines == null || pipelineIndex >= pipelines.size()) {
            return 0;
        }
        ...
        if (isIndividualPipelineStage(currentPipeline)) {
            Integer levelOrder = extractPipelineOrder(currentPipeline, currentLevel + 1);
            return sendPipelineIndividualApproverEmail(emailEntityManager, application, form, currentPipeline,
                    levelOrder, "Reminder: " + formName + " Pending Approval", reminderRemarks, isCapf) ? 1 : 0;
        }

        Integer departmentId = safeInt(currentPipeline.get("serDepartmentId"),
                safeInt(currentPipeline.get("departmentId"), null));
        String departmentName = resolveDepartmentName(emailEntityManager, departmentId, currentPipeline);
        if (isUserDepartmentHodStage(currentPipeline, departmentName)) {
            Integer submitterDeptId = loadUserDepartmentId(emailEntityManager, application.getSerSubmittedBy());
            if (submitterDeptId != null) {
                departmentId = submitterDeptId;
                departmentName = resolveDepartmentName(emailEntityManager, departmentId, null);
            }
        }
        Integer levelOrder = extractPipelineOrder(currentPipeline, currentLevel + 1);
        return sendDepartmentStageReminderEmail(emailEntityManager, application, form, departmentId, departmentName,
                levelOrder, formName, reminderRemarks, isCapf);
    }

    private int sendDepartmentStageReminderEmail(EntityManager emailEntityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form,
            Integer departmentId,
            String departmentName,
        ...
        try {
            ...
            HrTblDepartment department = emailEntityManager.find(HrTblDepartment.class, departmentId);
            if (department == null) {
                if (startedHere && emailEntityManager.getTransaction().isActive()) {
                    emailEntityManager.getTransaction().rollback();
                }
            ...
            java.util.List<Integer> headIds = new java.util.ArrayList<>();
            String headIdsStr = department.getSerDepartmentHeadId();
            if (headIdsStr != null && !headIdsStr.trim().isEmpty()) {
                for (String id : headIdsStr.split(",")) {
                    try {
                        headIds.add(Integer.parseInt(id.trim()));
            ...
            if (headIds.isEmpty()) {
                Integer fallbackHeadId = findDepartmentHeadUserId(emailEntityManager, departmentId);
                if (fallbackHeadId != null) {
                    headIds.add(fallbackHeadId);
                }
            }
            ...
            for (Integer headId : headIds) {
                ...
                String sendBackToInitiatorUrl = baseUrl + "/sendBackToInitiatorFromEmail?applicationId="
                        + application.getSerApplicationId() + "&userId=" + approver.getSerUserId();
                String subject = "Reminder: " + formName + " Pending Approval - Level " + levelOrder + " - "
                        + (application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A");
                String html = generateApprovalEmailHtml(
                        approver.getTxtUserName() != null ? approver.getTxtUserName() : "Department Head",
                        levelOrder,
                        application.getTxtFormCode() != null ? application.getTxtFormCode() : "N/A",
                        formName,
                        application.getTxtStatus(),
                        ...
                        baseUrl);
                sendEmailWithInlineFormPreview(
                        java.util.Collections.singletonList(approver.getTxtAddress()),
                        subject,
                        html,
                        application,
                        form,
            ...
            log.warn("Failed department-stage reminder email for appId={}: {}",
                    application.getSerApplicationId(), e.getMessage());
            return 0;
        }
        return sent;
                ...
                if (currentLevel != null && currentLevel == -1 && isCapf) {
                    ...
                    try {
                        ...
                        if (firstLevelPipeline != null) {
                            boolean capfInitiatorHodStage = isCapf && currentLevel != null && currentLevel == 0;
                            if (isIndividualPipelineStage(firstLevelPipeline) && !capfInitiatorHodStage) {
                                Integer firstLevelOrder = extractPipelineOrder(firstLevelPipeline, 1);
                                sendPipelineIndividualApproverEmail(emailEntityManager, application, form,
                                        firstLevelPipeline, firstLevelOrder, formName + " Pending Approval", null,
                                        isCapf);
                            } else {
                            emailEntityManager.getTransaction().begin();
    ...
    private static final int CAPF_CEO_SIGNATURE_PIPELINE_ORDER = 5;

    private Integer resolveCapfPipelineOrder(Map<String, Object> pipeline, int sortedIndex) {
        Integer order = safeInt(pipeline.get("intApprovalOrder"), null);
        return order != null ? order : (sortedIndex + 1);
    }

    private boolean isCapfPipelineCeoSignatureOrder(Integer order) {
        return order != null && order >= CAPF_CEO_SIGNATURE_PIPELINE_ORDER;
    }

    private boolean isCapfPipelineCeoSignatureStage(Map<String, Object> pipeline, int sortedIndex) {
        return isCapfPipelineCeoSignatureOrder(resolveCapfPipelineOrder(pipeline, sortedIndex));
    }

    private boolean capfHasPipelineCeoSignatureSlot(List<Map<String, Object>> pipelines) {
        if (pipelines == null || pipelines.isEmpty()) {
            return false;
        }
        java.util.List<Map<String, Object>> sorted = new java.util.ArrayList<>(pipelines);
        sorted.sort((a, b) -> resolveCapfPipelineOrder(a, 0).compareTo(resolveCapfPipelineOrder(b, 0)));
        for (int i = 0; i < sorted.size(); i++) {
            if (isCapfPipelineCeoSignatureStage(sorted.get(i), i)) {
                return true;
            }
        }
        return false;
    ...
    private void routeCapfAfterPipelineComplete(EntityManager entityManager,
            CfgTblCustomFormApplication application,
            CfgTblCustomForm form) {
        List<Map<String, Object>> pipelines = loadEffectiveApprovalPipeline(application, form, entityManager);
        if (capfHasPipelineCeoSignatureSlot(pipelines)) {
            application.setTxtStatus("ASSET_PENDING");
            Integer financeUserId = findFirstUserIdByRole(entityManager, "FINANCE_HEAD");
            if (financeUserId == null) {
                financeUserId = findFirstUserIdByRole(entityManager, "FINANCE");
    ...
    private List<Map<String, Object>>[] mapCapfApprovalEntries(List<Map<String, Object>> approved,
        ...
        if (pipelines != null && !pipelines.isEmpty()) {
            ...
            for (int i = 0; i < sorted.size(); i++) {
                ...
                Integer order = resolveCapfPipelineOrder(p, i);
                if (isCapfPipelineCeoSignatureOrder(order)) {
                    continue;
                }
                if (pipelineSlots.size() >= 4)
                    break;
        ...
        if (pipelines != null && !pipelines.isEmpty()) {
            java.util.List<Map<String, Object>> sorted = new java.util.ArrayList<>(pipelines);
            sorted.sort(
                    (a, b) -> resolveCapfPipelineOrder(a, 0).compareTo(resolveCapfPipelineOrder(b, 0)));
            for (int i = 0; i < sorted.size(); i++) {
                Map<String, Object> pipeline = sorted.get(i);
                Integer order = resolveCapfPipelineOrder(pipeline, i);
                if (!isCapfPipelineCeoSignatureOrder(order)) {
                    continue;
                }
                for (int j = 0; j < approved.size(); j++) {
                    ...
                    Map<String, Object> entry = approved.get(j);
                    Integer entryOrder = safeInt(entry.get("intApprovalOrder"), safeInt(entry.get("level"), null));
                    if (order != null && entryOrder != null && order.equals(entryOrder)) {
                        mapped[5].add(entry);
                        usedIndexes.add(j);
                        collectSameDeptEntries(mapped[5], entry, approved, usedIndexes);
                    }
                }
            }
        }
    ...
    private String extractDepartmentName(Object obj) {
        if (!(obj instanceof Map)) {
            return null;
        }
        Map<?, ?> map = (Map<?, ?>) obj;
        Object value = map.get("txtDepartmentName");
        if (value == null)
            value = map.get("departmentName");
        if (value == null)
            value = map.get("userDepartmentName");
        if (value == null) {
            Object hrDept = map.get("hrTblDepartment");
            if (hrDept instanceof Map) {
                value = ((Map<?, ?>) hrDept).get("txtDepartmentName");
                if (value == null)
                    value = ((Map<?, ?>) hrDept).get("departmentName");
            }
        }
        return value != null ? String.valueOf(value) : null;
    }

    private String extractDesignation(Object obj) {
        if (!(obj instanceof Map)) {
            return null;
        }
        Map<?, ?> map = (Map<?, ?>) obj;
        Object value = map.get("txtDesignation");
        if (value == null)
            value = map.get("designation");
        return value != null ? String.valueOf(value) : null;
    }
    ...
    private BufferedImage removeNearWhiteBackground(BufferedImage source, int threshold) {
        ...
        for (int py = 0; py < h; py++) {
            for (int px = 0; px < w; px++) {
                ...
                if (r >= threshold && g >= threshold && b >= threshold) {
                    argb.setRGB(px, py, 0x00FFFFFF);
                } else {
                    argb.setRGB(px, py, (rgb & 0x00FFFFFF) | 0xFF000000);
                }
            }
        }
        return argb;
    ...
    private PDImageXObject loadSignatureImageForOverlay(PDDocument document, File sigFile) throws java.io.IOException {
        BufferedImage raw = ImageIO.read(sigFile);
        if (raw == null) {
            return PDImageXObject.createFromFile(sigFile.getAbsolutePath(), document);
        }
        BufferedImage processed = trimTransparentPadding(removeNearWhiteBackground(raw, 242));
        return LosslessFactory.createFromImage(document, processed);
    }
    ...
    private void drawSignatureImage(PDDocument document, PDPageContentStream content, String signaturePath,
        ...
        try {
            ...
            if (sigFile == null || !sigFile.exists())
                return;
            PDImageXObject img = atBottom
                    ? loadSignatureImageForOverlay(document, sigFile)
                    : PDImageXObject.createFromFile(sigFile.getAbsolutePath(), document);
            ...
            float drawX = x + Math.max(0f, (maxWidth - drawW) / 2f);
    ...
    private String generateCapfEmailFragment(CfgTblCustomFormApplication application, CfgTblCustomForm form,
        ...
        String applicationCodesHtml = buildCapfApplicationCodesHtml(application);
    ...
    private String buildCapfApplicationCodesHtml(CfgTblCustomFormApplication application) {
        if (application == null) {
            return "";
        }

        StringBuilder html = new StringBuilder();
        appendCapfCodeBadge(html, "Asset Code", application.getTxtAssetCode());
        appendCapfCodeBadge(html, "PR Code", application.getTxtPrCode());
        return html.toString();
    }

    private void appendCapfCodeBadge(StringBuilder html, String label, String value) {
        if (html == null || value == null || value.trim().isEmpty()) {
            return;
        }

        html.append("<div style='margin-bottom:6px;'>")
                .append("<span style='display:inline-block;border:1px solid rgba(0,0,0,0.3);background:rgba(34,197,94,0.10);color:#166534;padding:4px 12px;font-size:12px;font-weight:700;'>")
    ...
    private String buildCapfSignatureSlotsHtml(List<Map<String, Object>> pipelines, String approvalHistoryJson,
        ...
        if (sortedPipelines.isEmpty()) {
            ...
            for (Map<String, Object> pipeline : sortedPipelines) {
                index++;
                Integer order = resolveCapfPipelineOrder(pipeline, index - 1);
                if (hasPipelineCeoSlot && isCapfPipelineCeoSignatureOrder(order)) {
                    continue;
                }
                Integer departmentId = null;
                ...
                String slotLabel = (order != null && order >= 1 && order < pipelineSlotLabels.length)
                        ? pipelineSlotLabels[order]
                        : (label != null && !label.trim().isEmpty() ? label : ("Department " + order));
    ...
    private CapfSignatureSlot buildCapfPipelineCeoSlot(List<Map<String, Object>> pipelines,
        ...
        if (pipelines != null) {
            ...
            for (int i = 0; i < sorted.size(); i++) {
                ...
                Integer departmentId = extractPipelineDepartmentId(pipeline);
                String label = extractPipelineDepartmentName(pipeline);
                if (hideFromOrder == null || order == null || order < hideFromOrder) {
                    entries.addAll(getApprovalEntriesForPipeline(order, departmentId, label, approvalHistory,
                            usedIndices));
                }
            }
        }
    ...
    private String generateApprovalEmailHtml(String recipientName, Integer level, String applicationCode,
        ...
        if (showActionButtons) {
            if ("SENT_BACK".equalsIgnoreCase(status) || "REVISION_REQUIRED".equalsIgnoreCase(status)) {
                html.append("<p style='margin:0 0 15px 0;'>This application has been <strong>sent back to you</strong> and now requires your action at Level ")
                        .append(level).append(".</p>");
            } else if ("SENT_BACK_TO_INITIATOR".equalsIgnoreCase(status)) {
                html.append("<p style='margin:0 0 15px 0;'>This application has been <strong>sent back to you as initiator</strong> and now requires your review.</p>");
            } else {
                html.append("<p style='margin:0 0 15px 0;'>A new application is pending your approval at Level ").append(level).append(".</p>");
            }
        } else {
            if ("SUBMITTED".equalsIgnoreCase(status)) {
                html.append("<p style='margin:0 0 15px 0;'>Your application has been submitted successfully and is now pending approval.</p>");
            } else if ("RESUBMITTED_BY_INITIATOR".equalsIgnoreCase(status)) {
                html.append("<p style='margin:0 0 15px 0;'>Your application has been resubmitted successfully and is now pending approval.</p>");
            } else if ("SENT_BACK_TO_INITIATOR".equalsIgnoreCase(status)) {
                html.append("<p style='margin:0 0 15px 0;'>Your application has been <strong>sent back to you</strong>.</p>");
                html.append("<p style='margin:0 0 15px 0;'>Please review the remarks and update your application accordingly.</p>");
            } else if ("SENT_BACK".equalsIgnoreCase(status) || "REVISION_REQUIRED".equalsIgnoreCase(status)) {
        ...
        String displayStatus = resolveEmailStatusBadgeText(status);
        String statusClass = displayStatus.toLowerCase(Locale.ROOT).replace("_", "-");
    ...
    private String resolveEmailStatusBadgeText(String status) {
        String normalized = status != null ? status.trim().toUpperCase(Locale.ROOT) : "";
        if ("SENT_BACK_TO_INITIATOR".equals(normalized)) {
            return "PENDING";
        }
        if ("SUBMITTED".equals(normalized) || "RESUBMITTED_BY_INITIATOR".equals(normalized)
                || "SENT_BACK".equals(normalized) || "REVISION_REQUIRED".equals(normalized)) {
            return "IN_PROGRESS";
        }
        if (normalized.isEmpty()) {
            return "PENDING";
    ...
    private void drawDynamicBudgetSignatureTable(PDPageContentStream content,
        ...
        content.setFont(PDType1Font.HELVETICA_BOLD, 9);
        float headerFontSize = 9f;
        // Vertically center header text in the header band with padding from borders.
        float headerY = y + rowSig + (rowHeader - headerFontSize) / 2f + 7f;
        ...
        float sigBandTop = y + rowSig + rowHeader;
        float sigBandHeight = Math.max(rowNames, 24f);
        float sigRowHeight = Math.min(28f, sigBandHeight - 8f);
        float sigTopPad = 6f;
        float sigRowY = sigBandTop + sigBandHeight - sigRowHeight - sigTopPad;
        for (int i = 0; i < cols; i++) {
        ...
        for (int i = 0; i < cols; i++) {
            ...
            float textBlockHeight = lines.size() * bodyLineHeight;
            // Bottom row: top-align user text below the header line, away from the lower cell border.
            float bottomPadding = 12f;
            float ty = y + rowSig - bottomPadding - textBlockHeight;
            for (String line : lines) {
                content.beginText();
                content.newLineAtOffset(tx, ty);
                content.showText(line);
    ...
    private byte[] applyDynamicFooterSignaturesToPdf(byte[] pdfBytes, Map<String, Object> appData,
        ...
        return applyDynamicFooterSignaturesToPdf(pdfBytes, appData, approvalHistoryJson, onlyLastEntry, 0f);
    }

    private byte[] applyDynamicFooterSignaturesToPdf(byte[] pdfBytes, Map<String, Object> appData,
            String approvalHistoryJson, boolean onlyLastEntry, float signatureLift) {
        if (pdfBytes == null || pdfBytes.length == 0) {
            return null;
        }
        List<Map<String, Object>> footerFields = extractFooterFields(appData);
        ...
        try (PDDocument document = PDDocument.load(pdfBytes)) {
            ...
            boolean landscapePage = pageWidth > pageHeight;
            boolean landscapeEmail = signatureLift > 0f && landscapePage;
            float margin = 40f;
            float tableX = margin;
            float tableWidth = pageWidth - margin * 2;
            if (signatureLift > 0f) {
                float[] layout = emailFooterTableLayout(pageWidth);
                tableX = layout[0];
                tableWidth = layout[1];
            }
            float tableBottomY = landscapeEmail
                    ? EMAIL_DYNAMIC_FOOTER_LANDSCAPE_TABLE_BOTTOM_Y
                    : EMAIL_DYNAMIC_FOOTER_TABLE_BOTTOM_Y;
            float effectiveSignatureLift = signatureLift;
            if (landscapeEmail) {
                effectiveSignatureLift += EMAIL_DYNAMIC_FOOTER_LANDSCAPE_EXTRA_SIGNATURE_LIFT;
            }
            float tableHeight = 110f;
            try (PDPageContentStream content = new PDPageContentStream(
                    document,
    ...
    /**
     * Y layout for signature overlay on individual-pipeline footer PDFs.
     * Returns [sigAreaY, sigAreaHeight, timestampBaselineY, maxSigDrawHeight].
     * Signature image and timestamp are vertically centered together inside the top signature blank row.
     */
    private float[] computeDynamicFooterOverlaySignatureYs(float tableBottomY, float tableHeight,
            float signatureLift, boolean landscapeEmail) {
        float rowSig = 50f;
        float rowHeader = 22f;
        float rowNames = tableHeight - rowSig - rowHeader;
        float sigRowBottom = tableBottomY + rowNames + rowHeader;
        float sigRowTop = tableBottomY + tableHeight;

        float bottomPad = landscapeEmail ? 6f : 5f;
        float topPad = landscapeEmail ? 6f : 5f;
        float timestampTextHeight = 8f;
        float gapSigTime = 2f;
        float maxSigDrawHeight = landscapeEmail ? 15f : 16f;

        float innerBottom = sigRowBottom + bottomPad + signatureLift;
        float innerTop = sigRowTop - topPad + signatureLift;
        float availableHeight = Math.max(0f, innerTop - innerBottom);

        float sigAreaHeight = Math.min(maxSigDrawHeight,
                Math.max(10f, availableHeight - timestampTextHeight - gapSigTime));
        float stackHeight = sigAreaHeight + gapSigTime + timestampTextHeight;
        float stackOffset = Math.max(0f, (availableHeight - stackHeight) / 2f);
        float timestampBaselineY = innerBottom + stackOffset + 1.5f;
        float sigAreaY = timestampBaselineY + timestampTextHeight + gapSigTime;

        return new float[] { sigAreaY, sigAreaHeight, timestampBaselineY, sigAreaHeight };
    }

    private float sigRowBottomY(float tableBottomY, float tableHeight) {
        float rowSig = 50f;
        float rowHeader = 22f;
        float rowNames = tableHeight - rowSig - rowHeader;
        return tableBottomY + rowNames + rowHeader;
    }

    private float sigRowTopY(float tableBottomY, float tableHeight) {
        return tableBottomY + tableHeight;
    }

    private void clearSignatureImageArea(PDPageContentStream content, float x, float y, float width, float height)
            throws java.io.IOException {
        content.saveGraphicsState();
        content.setNonStrokingColor(255, 255, 255);
        content.addRect(x, y - 1f, width, height + 3f);
        content.fill();
    ...
    private void drawDynamicFooterSignaturesOnly(PDPageContentStream content,
            ...
            PDDocument document,
            float signatureLift,
            boolean landscapeEmail) throws java.io.IOException {
        ...
        float[] overlayYs = computeDynamicFooterOverlaySignatureYs(y, height, signatureLift, landscapeEmail);
        float sigAreaY = overlayYs[0];
        float sigAreaHeight = overlayYs[1];
        float timestampY = overlayYs[2];
        float maxSigDrawHeight = overlayYs[3];
        int slotIndex = 0;
        for (int i = 0; i < sections; i++) {
            Map<String, Object> field = footerFields.get(i);
            String role = field != null && field.get("label") != null ? String.valueOf(field.get("label")) : "APPROVER";
            ...
            for (Object userObj : users) {
                ...
                if (sigPath != null && !sigPath.trim().isEmpty()) {
                    float[] cell = dynamicFooterCellBounds(x, colWidth, slotIndex);
                    float cellX = cell[0];
                    float cellW = cell[1];
                    drawSignatureImage(document, content, sigPath, cellX, sigAreaY, cellW, sigAreaHeight, false,
                            maxSigDrawHeight);
                    ...
                    if (entry != null) {
                        ...
                        if (dateTimeStr != null && !dateTimeStr.isEmpty()) {
                            drawTimestampBelowSignature(content, cellX, timestampY, cellW, dateTimeStr);
                        }
                    }
                }
                slotIndex++;
    ...
    private void drawDynamicFooterSignatureLastEntryOnly(PDPageContentStream content,
        ...
        float timestampY = overlayYs[2];
        float maxSigDrawHeight = overlayYs[3];
        float[] cell = dynamicFooterCellBounds(x, colWidth, targetSlotIndex);
        float cellX = cell[0];
        float cellW = cell[1];
        drawSignatureImage(document, content, sigPath, cellX, sigAreaY, cellW, sigAreaHeight, false,
                maxSigDrawHeight);
    ...
    private List<Map<String, Object>> loadEffectiveApprovalPipeline(CfgTblCustomFormApplication application,
        ...
        if (isTemplateBuilderApplication(appData)) {
            List<Map<String, Object>> footerFields = extractFooterFields(appData);
            if (footerFields != null && !footerFields.isEmpty()) {
                List<Map<String, Object>> footerPipeline = new java.util.ArrayList<>();
                int order = 2;
                for (Map<String, Object> field : footerFields) {
                    if (field == null) {
                        continue;
                    }
                    String key = field.get("key") != null ? String.valueOf(field.get("key")).trim() : "";
                    ...
                    java.util.LinkedHashSet<Integer> userIds = new java.util.LinkedHashSet<>();
                    java.util.List<Map<String, Object>> normalizedUsers = new java.util.ArrayList<>();
                    for (Object userObj : users) {
                        Integer userId = extractUserId(userObj);
                        ...
                        if (userObj instanceof Map) {
                            @SuppressWarnings("unchecked")
                            Map<String, Object> castUser = new java.util.LinkedHashMap<>((Map<String, Object>) userObj);
                            normalizedUsers.add(castUser);
                        } else {
                            java.util.Map<String, Object> normalizedUser = new java.util.LinkedHashMap<>();
                            normalizedUser.put("serUserId", userId);
                    ...
                    step.put("id", !key.isEmpty() ? key : "footer_" + footerPipeline.size());
                    step.put("name", field.get("label") != null ? String.valueOf(field.get("label")) : "Approver");
                    step.put("type", "individual");
                    step.put("approvalMode",
                            "AND".equalsIgnoreCase(firstNonBlank(
                                    field.get("approvalMode") != null ? String.valueOf(field.get("approvalMode")) : null,
                                    field.get("condition") != null ? String.valueOf(field.get("condition")) : null))
                                    ? "AND"
                                    : "OR");
                    step.put("users", normalizedUsers);
                    step.put("userIds", new java.util.ArrayList<>(userIds));
                    step.put("intApprovalOrder", order++);
                    footerPipeline.add(step);
                }
                if (!footerPipeline.isEmpty()) {
    ...
    private byte[] resolveBestPdfBytesForEmail(CfgTblCustomFormApplication application, CfgTblCustomForm form) {
        try {
            ...
            // For individual pipeline footer forms, always use the stored workflow PDF first.
            // This keeps email previews aligned with the latest approval/send-back signatures
            // regardless of whether the action happened in the portal or through an email link.
            if (useIndividualPipelineFlow || hasDynamicFooterFlow) {
                if (application != null && application.getBlbPdfData() != null && application.getBlbPdfData().length > 0) {
                    return application.getBlbPdfData();
                }
                ...
                // If still no stored PDF, do not generate a backend substitute.
                // Sending without a preview is safer than sending a differently paginated document.
                log.warn("No stored PDF available for individual pipeline footer form in email, appId={}", 
            ...
            // For standard budget forms without dynamic footer workflow, regenerate on demand.
            if (isBudgetApproval) {
                return generateApplicationPdf(application, form, appData != null ? appData : new java.util.HashMap<>());
            }
    ...
    private byte[] resolveStageZeroPdfForEmailPreview(CfgTblCustomFormApplication application) {
        if (application == null) {
            return null;
        }
        byte[] stageZeroPdf = application.getBlbPdfForStage(0);
        ...
        EntityManager em = getEntityManager();
        try {
            if (application.getSerApplicationId() != null) {
                CfgTblCustomFormApplication dbApp = em.find(CfgTblCustomFormApplication.class,
                        application.getSerApplicationId());
                if (dbApp != null) {
                    stageZeroPdf = dbApp.getBlbPdfForStage(0);
                    if (stageZeroPdf != null && stageZeroPdf.length > 0) {
                        return stageZeroPdf;
                    }
                }
    ...
    private byte[] buildGeneralFooterEmailPreviewPdf(CfgTblCustomFormApplication application,
            CfgTblCustomForm form,
            byte[] fallbackPdfBytes) {
        try {
            if (application == null || isCapfForm(form) || isBudgetApprovalForm(form)
                    || !hasDynamicFooterFlow(application)) {
                return fallbackPdfBytes;
            }
            Map<String, Object> appData = parseApplicationData(application);
            if (extractFooterFields(appData).isEmpty()) {
                return fallbackPdfBytes;
            }
            byte[] basePdf = resolveStageZeroPdfForEmailPreview(application);
            if (basePdf == null || basePdf.length == 0) {
                return fallbackPdfBytes;
            }
            if (fallbackPdfBytes != null && fallbackPdfBytes.length > 0) {
                int fallbackPages = getPdfPageCount(fallbackPdfBytes);
                int basePages = getPdfPageCount(basePdf);
                if (fallbackPages > basePages) {
                    return fallbackPdfBytes;
                }
            }
            byte[] previewPdf = applyDynamicFooterSignaturesToPdf(
                    basePdf,
                    appData,
                    application.getTxtApprovalHistory(),
                    false,
                    EMAIL_DYNAMIC_FOOTER_SIGNATURE_LIFT);
            return previewPdf != null && previewPdf.length > 0 ? previewPdf : fallbackPdfBytes;
        } catch (Exception e) {
            log.warn("Unable to build email-only dynamic-footer preview PDF: {}", e.getMessage());
            return fallbackPdfBytes;
        }
    }
    ...
    private void sendEmailWithInlineFormPreview(List<String> recipients, String subject, String html,
        ...
        try {
            ...
            // IMPORTANT: use the same stored PDF bytes that are attached to the email.
            // Do not rebuild a separate email-only preview PDF, otherwise pagination can drift
            // from `/application-details` and from the attached document.
    ...
    private void sendBackEmailNotification(CfgTblCustomFormApplication application, Integer oldLevelBeforeSendBack,
        ...
        try {
            ...
            if (useIndividualPipelineFlow && newLevelAfterSendBack >= 0) {
                List<BudgetApprover> previousApprovers = resolveIndividualFooterApproversForLevel(
                        emailEntityManager,
                        application,
                        form,
                        appData,
                        pipelines,
                        newLevelAfterSendBack);
                java.util.List<BudgetApprover> deliverableApprovers = new java.util.ArrayList<>();
                for (BudgetApprover previousApprover : previousApprovers) {
                    if (previousApprover != null && previousApprover.userId != null && previousApprover.email != null
                            && !previousApprover.email.trim().isEmpty()) {
                        deliverableApprovers.add(previousApprover);
                    }
                }
                if (!deliverableApprovers.isEmpty()) {
                        String baseUrl = getBaseUrl();
                        boolean isCapf = isCapfForm(form);
                        String cid = isCapf ? "capf-inline" : "form-inline";
                        ...
                        for (BudgetApprover previousApprover : deliverableApprovers) {
                            String approveUrl = baseUrl + "/approveApplicationFromEmail?applicationId="
                            ...
                            String html = generateApprovalEmailHtml(
                                previousApprover.name != null && !previousApprover.name.trim().isEmpty()
                                        ? previousApprover.name.trim()
                                        : "User",
                            ...
                            sentEmails.add(previousApprover.email);
                        }
                        
                        log.info("Send-back notification email sent to previous approver step (individual pipeline): {} at level {}", 
                            sentEmails, newLevelAfterSendBack);
            ...
            if (targetDeptId == null) {
                ...
                if (isIndividualPipelineStage(targetPipeline)) {
                    targetUserId = extractPipelineUserId(targetPipeline);
                }
    ...
    private Integer capfHistoryLevelForStage(boolean isCapf, Integer currentLevel, Integer pipelineOrder) {
        int level = currentLevel != null ? currentLevel : 0;
        if (isCapf && level == 0) {
            return 0;
        }
        return pipelineOrder != null ? pipelineOrder : level + 1;
    }

    private boolean capfUserAlreadyApprovedOpenStage(CfgTblCustomFormApplication application, boolean isCapf,
            Integer currentLevel, Integer pipelineOrder, Integer departmentId, String departmentName, Integer userId) {
        if (!isCapf || application == null) {
            return false;
        }
        Integer stageLevel = capfHistoryLevelForStage(true, currentLevel, pipelineOrder);
        List<Map<String, Object>> approvalHistory = parseApprovalHistory(application.getTxtApprovalHistory());
        return userAlreadyApprovedStage(approvalHistory, userId, stageLevel, departmentId, departmentName);
    }

    private boolean userAlreadyApprovedStage(List<Map<String, Object>> approvalHistory, Integer userId,
            Integer level, Integer departmentId, String departmentName) {
        if (approvalHistory == null || approvalHistory.isEmpty() || userId == null || level == null) {
            return false;
        }
        long cutoffEpoch = getLatestResetEpochForStage(approvalHistory, level);
        for (int i = 0; i < approvalHistory.size(); i++) {
            ...
            Integer approvedBy = extractApprovalUserId(entry);
            if (approvedBy == null || !approvedBy.equals(userId)) {
                continue;
            }
    ...
    private boolean approvalEntryMatchesStage(Map<String, Object> entry, Integer departmentId, String departmentName) {
        if (entry == null) {
            return false;
        }
        Integer entryDeptId = safeInt(entry.get("departmentId"), safeInt(entry.get("serDepartmentId"), null));
        if (departmentId != null) {
            if (departmentId.equals(entryDeptId)) {
                return true;
            }
            return entryDeptId == null && approvalEntryMatchesStageName(entry, departmentName);
        }

        String expectedName = normalizeDeptText(departmentName);
        if (expectedName.isEmpty()) {
            return true;
        }
        return approvalEntryMatchesStageName(entry, departmentName);
    }

    private boolean approvalEntryMatchesStageName(Map<String, Object> entry, String departmentName) {
        String expectedName = normalizeDeptText(departmentName);
        if (expectedName.isEmpty()) {
            return false;
        }
        String entryDepartmentName = normalizeDeptText(
                entry.get("departmentName") != null ? String.valueOf(entry.get("departmentName")) : "");
        String entryRole = normalizeDeptText(entry.get("role") != null ? String.valueOf(entry.get("role")) : "");
        return expectedName.equals(entryDepartmentName) || expectedName.equals(entryRole);
    }
